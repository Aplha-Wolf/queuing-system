...
-opaque handler(PKB) :: {handler,
        fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> PKB)}.
...
-spec expect_ok_response(
    fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> PKZ)
) -> handler(PKZ).
...
-spec expect_json_response(
    fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> PKL)
) -> handler(PKL).
...
-spec expect_text_response(
    fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> PKU)
) -> handler(PKU).
...
-spec expect_text(fun(({ok, binary()} | {error, error()}) -> PKP)) -> handler(PKP).
expect_text(Handler) ->
...
-spec expect_any_response(
    fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> PLE)
) -> handler(PLE).
expect_any_response(Handler) ->
    {handler, Handler}.

-file("src/rsvp.gleam", 403).
-spec do_send(gleam@http@request:request(binary()), handler(PMA)) -> lustre@effect:effect(PMA).
...
-spec send(gleam@http@request:request(binary()), handler(PLW)) -> lustre@effect:effect(PLW).
send(Request, Handler) ->
    do_send(Request, Handler).

-file("src/rsvp.gleam", 460).
-spec do_send_bits(gleam@http@request:request(bitstring()), handler(PMI)) -> lustre@effect:effect(PMI).
...
-spec send_bits(gleam@http@request:request(bitstring()), handler(PME)) -> lustre@effect:effect(PME).
...
-spec simulate(
    lustre@dev@simulate:simulation(PML, PMM),
...
-spec reject(error(), handler(PMT)) -> lustre@effect:effect(PMT).
reject(Err, Handler) ->
...
-spec expect_json(
    gleam@dynamic@decode:decoder(PKC),
    fun(({ok, PKC} | {error, error()}) -> PKG)
) -> handler(PKG).
...
-spec get(binary(), handler(PLG)) -> lustre@effect:effect(PLG).
get(Url, Handler) ->
    case to_uri(Url) of
        {ok, Uri} ->
...
-spec post(binary(), gleam@json:json(), handler(PLJ)) -> lustre@effect:effect(PLJ).
...
-spec put(binary(), gleam@json:json(), handler(PLM)) -> lustre@effect:effect(PLM).
...
-spec patch(binary(), gleam@json:json(), handler(PLP)) -> lustre@effect:effect(PLP).
...
-spec delete(binary(), gleam@json:json(), handler(PLS)) -> lustre@effect:effect(PLS).
