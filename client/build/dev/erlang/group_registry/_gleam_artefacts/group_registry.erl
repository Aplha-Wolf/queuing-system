...
-type group_registry(OHX) :: any() | {gleam_phantom, OHX}.

-type message(OHY) :: any() | {gleam_phantom, OHY}.

-type do_not_leak() :: any().

-file("src/group_registry.gleam", 104).
-spec get_registry(gleam@erlang@process:name(message(OIJ))) -> group_registry(OIJ).
get_registry(Name) ->
...
-spec start(gleam@erlang@process:name(message(OHZ))) -> {ok,
        gleam@otp@actor:started(group_registry(OHZ))} |
...
-spec supervised(gleam@erlang@process:name(message(OIE))) -> gleam@otp@supervision:child_specification(group_registry(OIE)).
supervised(Name) ->
...
-spec make_subject(group_registry(OIX), binary(), gleam@erlang@process:pid_()) -> gleam@erlang@process:subject(OIX).
...
-spec join(group_registry(OIN), binary(), gleam@erlang@process:pid_()) -> gleam@erlang@process:subject(OIN).
...
-spec members(group_registry(OIT), binary()) -> list(gleam@erlang@process:subject(OIT)).
