...
-opaque next(ABOV, ABOW) :: {continue,
        ABOV,
        gleam@option:option(gleam@erlang@process:selector(ABOW))} |
    normal_stop |
...
-opaque builder(ABOX, ABOY) :: {builder,
        integer(),
        fun((gleam@http@request:request(ABOX)) -> gleam@http@response:response(ABOY)),
...
-type websocket_message(ABOZ) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, ABOZ}.
...
-spec continue(ABPA) -> next(ABPA, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 58).
-spec with_selector(next(ABPE, ABPF), gleam@erlang@process:selector(ABPF)) -> next(ABPE, ABPF).
...
-spec convert_next(next(ABPT, ABPU)) -> mist@internal@next:next(ABPT, ABPU).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(ABQV)) -> gleam@http@response:response(ABQX))
) -> builder(ABQV, ABQX).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(ABRB, ABRC), integer()) -> builder(ABRB, ABRC).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), ABRH),
    integer(),
    gleam@http@response:response(ABRH)
) -> builder(mist@internal@http:connection(), ABRH).
...
-spec bind(builder(ABRT, ABRU), binary()) -> builder(ABRT, ABRU).
...
-spec with_ipv6(builder(ABRZ, ABSA)) -> builder(ABRZ, ABSA).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(ABSF, ABSG), binary(), binary()) -> builder(ABSF, ABSG).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(ABSV)
) -> {ok, websocket_message(ABSV)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((ABTB, websocket_message(ABTC), mist@internal@websocket:websocket_connection()) -> next(ABTB, ABTC)),
    fun((mist@internal@websocket:websocket_connection()) -> {ABTB,
        gleam@option:option(gleam@erlang@process:selector(ABTC))}),
    fun((ABTB) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(ABTQ)) -> {ok,
            gleam@otp@actor:initialised(ABTS, ABTQ, any())} |
        {error, binary()}),
    fun((ABTS, ABTQ, s_s_e_connection()) -> gleam@otp@actor:next(ABTS, ABTQ))
) -> gleam@http@response:response(response_data()).
