...
-type valid_message(ABAL) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, ABAL}.

-type websocket_message(ABAM) :: {valid, valid_message(ABAM)} | invalid.
...
-type handler_message(ABAN) :: {internal, gramps@websocket:frame()} |
    {user, ABAN}.

-type websocket_state(ABAO) :: {websocket_state,
        bitstring(),
        ABAO,
...
-spec map_user_selector(
    gleam@option:option(gleam@erlang@process:selector(ABBU))
) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(ABBU))).
...
-spec apply_frames(
    ...
    fun((ABBK, handler_message(ABBL), websocket_connection()) -> mist@internal@next:next(ABBK, ABBL)),
    websocket_connection(),
    mist@internal@next:next(ABBK, websocket_message(ABBL)),
    fun((ABBK) -> nil)
) -> mist@internal@next:next(ABBK, websocket_message(ABBL)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {ABAX,
        gleam@option:option(gleam@erlang@process:selector(ABAY))}),
    fun((ABAX) -> nil),
    fun((ABAX, handler_message(ABAY), websocket_connection()) -> mist@internal@next:next(ABAX, ABAY)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(websocket_message(ABAY)))} |
    {error, nil}.
