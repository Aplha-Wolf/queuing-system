...
-opaque cache(SBW) :: {cache,
        events(SBW),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(SBW)), lustre@vdom@vnode:element(SBW)),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(SBW)), lustre@vdom@vnode:element(SBW)),
        list(binary()),
        list(binary())}.

-opaque events(SBX) :: {events,
        lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(SBX))),
        lustre@internals@mutable_map:mutable_map(binary(), child(SBX))}.

-type child(SBY) :: {child,
        fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
        events(SBY)}.

-type added_children(SBZ) :: {added_children,
        lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(SBZ))),
        lustre@internals@mutable_map:mutable_map(binary(), child(SBZ)),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(SBZ)), lustre@vdom@vnode:element(SBZ))}.

-opaque decoded_event(SCA) :: {decoded_event,
        binary(),
        lustre@vdom@vattr:handler(SCA)} |
...
?DOC(false).
-spec events(cache(SCL)) -> events(SCL).
events(Cache) ->
    erlang:element(2, Cache).

-file("src/lustre/vdom/cache.gleam", 106).
?DOC(false).
-spec update_events(cache(SCO), events(SCO)) -> cache(SCO).
...
?DOC(false).
-spec memos(cache(SCS)) -> lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(SCS)), lustre@vdom@vnode:element(SCS)).
memos(Cache) ->
    erlang:element(3, Cache).
...
-spec get_old_memo(
    cache(SCV),
    fun(() -> lustre@vdom@vnode:element(SCV)),
    fun(() -> lustre@vdom@vnode:element(SCV))
...
-spec keep_memo(
    cache(SDA),
    fun(() -> lustre@vdom@vnode:element(SDA)),
    fun(() -> lustre@vdom@vnode:element(SDA))
) -> cache(SDA).
...
-spec add_memo(
    cache(SDF),
    fun(() -> lustre@vdom@vnode:element(SDF)),
    lustre@vdom@vnode:element(SDF)
) -> cache(SDF).
...
?DOC(false).
-spec do_add_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(SDW))),
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(SDW))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(SDW))).
...
-spec add_event(
    ...
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(SDR))
) -> events(SDR).
...
-spec do_remove_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(SEK))),
    lustre@vdom@path:path(),
    binary()
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(SEK))).
...
?DOC(false).
-spec remove_event(events(SEH), lustre@vdom@path:path(), binary()) -> events(SEH).
...
?DOC(false).
-spec add_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(SEZ))),
    lustre@vdom@path:path(),
...
-spec do_add_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(SFR))),
    lustre@internals@mutable_map:mutable_map(binary(), child(SFR)),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(SFR)), lustre@vdom@vnode:element(SFR)),
    lustre@vdom@path:path(),
    integer(),
...
-spec add_children(
    ...
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(SFK))
) -> {cache(SFK), events(SFK)}.
...
?DOC(false).
-spec from_node(lustre@vdom@vnode:element(SCF)) -> cache(SCF).
from_node(Root) ->
    Cache = new(),
...
-spec remove_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(SGI))),
    lustre@vdom@path:path(),
    list(lustre@vdom@vattr:attribute(SGI))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(SGI))).
...
-spec do_remove_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(SGT))),
    lustre@internals@mutable_map:mutable_map(binary(), child(SGT)),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(SGT)), lustre@vdom@vnode:element(SGT)),
    lustre@vdom@path:path(),
    integer(),
...
-spec replace_child(
    ...
    lustre@vdom@path:path(),
    integer(),
    lustre@vdom@vnode:element(SHF),
    lustre@vdom@vnode:element(SHF)
) -> {cache(SHF), events(SHF)}.
...
?DOC(false).
-spec dispatch(cache(SHT), decoded_event(SHT)) -> {cache(SHT),
    {ok, lustre@vdom@vattr:handler(SHT)} | {error, nil}}.
...
?DOC(false).
-spec handle(cache(SHX), binary(), binary(), gleam@dynamic:dynamic_()) -> {cache(SHX),
    {ok, lustre@vdom@vattr:handler(SHX)} | {error, nil}}.
