...
-type runtime(VDR) :: any() | {gleam_phantom, VDR}.
...
-spec application(
    fun((VEN) -> {VEO, lustre@effect:effect(VEP)}),
    fun((VEO, VEP) -> {VEO, lustre@effect:effect(VEP)}),
    fun((VEO) -> lustre@vdom@vnode:element(VEP))
) -> lustre@runtime@app:app(VEN, VEO, VEP).
...
-spec element(lustre@vdom@vnode:element(VEA)) -> lustre@runtime@app:app(any(), nil, VEA).
element(View) ->
    application(
...
-spec simple(
    fun((VEG) -> VEH),
    fun((VEH, VEI) -> VEH),
    fun((VEH) -> lustre@vdom@vnode:element(VEI))
) -> lustre@runtime@app:app(VEG, VEH, VEI).
...
-spec component(
    fun((VEW) -> {VEX, lustre@effect:effect(VEY)}),
    fun((VEX, VEY) -> {VEX, lustre@effect:effect(VEY)}),
    fun((VEX) -> lustre@vdom@vnode:element(VEY)),
    list(lustre@runtime@app:option(VEY))
) -> lustre@runtime@app:app(VEW, VEX, VEY).
...
-spec named(
    lustre@runtime@app:app(VFH, VFI, VFJ),
    gleam@erlang@process:name(lustre@runtime@server@runtime:message(VFJ))
) -> lustre@runtime@app:app(VFH, VFI, VFJ).
named(App, Name) ->
    {app,
        {some, Name},
...
-spec do_start(lustre@runtime@app:app(VGB, any(), VGD), binary(), VGB) -> {ok,
        runtime(VGD)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.
...
-spec supervised(lustre@runtime@app:app(VGT, any(), VGV), VGT) -> gleam@otp@supervision:child_specification(gleam@erlang@process:subject(lustre@runtime@server@runtime:message(VGV))).
...
-spec factory(lustre@runtime@app:app(VHC, any(), VHE)) -> gleam@otp@factory_supervisor:builder(VHC, gleam@erlang@process:subject(lustre@runtime@server@runtime:message(VHE))).
factory(App) ->
...
-spec send(runtime(VHT), lustre@runtime@server@runtime:message(VHT)) -> nil.
...
-spec dispatch(VHW) -> lustre@runtime@server@runtime:message(VHW).
dispatch(Message) ->
...
-spec start(lustre@runtime@app:app(VFS, any(), VFU), binary(), VFS) -> {ok,
        runtime(VFU)} |
    {error, error()}.
...
-spec start_server_component(lustre@runtime@app:app(VGK, any(), VGM), VGK) -> {ok,
        runtime(VGM)} |
    {error, error()}.
