...
-opaque effect(PMZ) :: {effect,
        list(fun((actions(PMZ)) -> nil)),
        list(fun((actions(PMZ)) -> nil)),
        list(fun((actions(PMZ)) -> nil))}.

-type actions(PNA) :: {actions,
        fun((PNA) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(PNA)) -> nil),
...
-spec do_comap_select(
    actions(POK),
    gleam@erlang@process:selector(POM),
    fun((POM) -> POK)
) -> nil.
...
-spec do_comap_actions(actions(POG), fun((POI) -> POG)) -> actions(POI).
...
-spec do_map(list(fun((actions(POA)) -> nil)), fun((POA) -> POD)) -> list(fun((actions(POD)) -> nil)).
do_map(Effects, F) ->
    gleam@list:map(
        Effects,
        fun(Effect) ->
...
-spec map(effect(PNW), fun((PNW) -> PNY)) -> effect(PNY).
map(Effect, F) ->
    {effect,
...
-spec perform(
    ...
    fun((POO) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(POO)) -> nil),
    fun(() -> gleam@dynamic:dynamic_()),
    fun((binary(), gleam@json:json()) -> nil)
) -> nil.
...
-spec from(fun((fun((PND) -> nil)) -> nil)) -> effect(PND).
from(Effect) ->
    Task = fun(Actions) ->
...
-spec before_paint(fun((fun((PNF) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(PNF).
before_paint(Effect) ->
    Task = fun(Actions) ->
...
-spec after_paint(fun((fun((PNH) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(PNH).
after_paint(Effect) ->
    Task = fun(Actions) ->
        Root = (erlang:element(5, Actions))(),
        Dispatch = erlang:element(2, Actions),
...
?DOC(false).
-spec select(
    fun((fun((PNL) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(PNL))
) -> effect(PNL).
select(Sel) ->
    Task = fun(Actions) ->
...
-spec batch(list(effect(PNS))) -> effect(PNS).
batch(Effects) ->
    gleam@list:fold(
        Effects,
        {effect, [], [], []},
