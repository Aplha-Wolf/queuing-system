...
-type element(PWQ) :: {fragment,
        integer(),
        binary(),
        list(element(PWQ)),
        lustre@internals@mutable_map:mutable_map(binary(), element(PWQ))} |
    {element,
        integer(),
        binary(),
        binary(),
        binary(),
        list(lustre@vdom@vattr:attribute(PWQ)),
        list(element(PWQ)),
        lustre@internals@mutable_map:mutable_map(binary(), element(PWQ)),
        boolean(),
        boolean()} |
...
?DOC(false).
-spec to_keyed(binary(), element(PYE)) -> element(PYE).
to_keyed(Key, Node) ->
    case Node of
...
-spec fragment(
    binary(),
    list(element(PWY)),
    lustre@internals@mutable_map:mutable_map(binary(), element(PWY))
) -> element(PWY).
...
-spec element(
    ...
    list(element(PXF)),
    lustre@internals@mutable_map:mutable_map(binary(), element(PXF)),
    boolean(),
    boolean()
) -> element(PXF).
element(
...
?DOC(false).
-spec map(element(PXU), fun((PXU) -> PXW)) -> element(PXW).
map(Element, Mapper) ->
    case Element of
...
?DOC(false).
-spec memo(binary(), list(lustre@internals@ref:ref()), fun(() -> element(PXZ))) -> element(PXZ).
...
-spec fragment_to_json(
    ...
    binary(),
    list(element(PYH)),
    lustre@internals@mutable_map:mutable_map(fun(() -> element(PYH)), element(PYH))
) -> gleam@json:json().
...
-spec element_to_json(
    ...
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(PYH)),
    list(element(PYH)),
    lustre@internals@mutable_map:mutable_map(fun(() -> element(PYH)), element(PYH))
) -> gleam@json:json().
