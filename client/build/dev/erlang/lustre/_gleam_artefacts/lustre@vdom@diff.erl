...
-type diff(SSQ) :: {diff,
        lustre@vdom@patch:patch(SSQ),
        lustre@vdom@cache:cache(SSQ)}.

-type partial_diff(SSR) :: {partial_diff,
        lustre@vdom@patch:patch(SSR),
        lustre@vdom@cache:cache(SSR),
        lustre@vdom@cache:events(SSR)}.

-type attribute_change(SSS) :: {attribute_change,
        list(lustre@vdom@vattr:attribute(SSS)),
        list(lustre@vdom@vattr:attribute(SSS)),
        lustre@vdom@cache:events(SSS)}.
...
-spec diff_attributes(
    boolean(),
    lustre@vdom@path:path(),
    lustre@vdom@cache:events(STV),
    list(lustre@vdom@vattr:attribute(STV)),
    list(lustre@vdom@vattr:attribute(STV)),
    list(lustre@vdom@vattr:attribute(STV)),
    list(lustre@vdom@vattr:attribute(STV))
) -> attribute_change(STV).
...
?DOC(false).
-spec do_diff(
    list(lustre@vdom@vnode:element(SSY)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(SSY)),
    list(lustre@vdom@vnode:element(SSY)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(SSY)),
    ...
    integer(),
    integer(),
    list(lustre@vdom@patch:change(SSY)),
    list(lustre@vdom@patch:patch(SSY)),
    lustre@vdom@path:path(),
    lustre@vdom@cache:cache(SSY),
    lustre@vdom@cache:events(SSY)
) -> partial_diff(SSY).
do_diff(
    Old,
    Old_keyed,
...
?DOC(false).
-spec diff(
    lustre@vdom@cache:cache(SST),
    lustre@vdom@vnode:element(SST),
    lustre@vdom@vnode:element(SST)
) -> diff(SST).
diff(Cache, Old, New) ->
