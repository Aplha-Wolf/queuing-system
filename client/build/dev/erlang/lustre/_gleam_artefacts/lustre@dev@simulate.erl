...
-opaque app(WBT, WBU, WBV) :: {app,
        fun((WBT) -> {WBU, lustre@effect:effect(WBV)}),
        fun((WBU, WBV) -> {WBU, lustre@effect:effect(WBV)}),
        fun((WBU) -> lustre@vdom@vnode:element(WBV))}.

-opaque simulation(WBW, WBX) :: {simulation,
        fun((WBW, WBX) -> {WBW, lustre@effect:effect(WBX)}),
        fun((WBW) -> lustre@vdom@vnode:element(WBX)),
        list(event(WBX)),
        WBW,
        lustre@vdom@vnode:element(WBX)}.

-type event(WBY) :: {dispatch, WBY} |
...
-spec simple(
    fun((WBZ) -> WCA),
    fun((WCA, WCB) -> WCA),
    fun((WCA) -> lustre@vdom@vnode:element(WCB))
) -> app(WBZ, WCA, WCB).
...
-spec application(
    fun((WCG) -> {WCH, lustre@effect:effect(WCI)}),
    fun((WCH, WCI) -> {WCH, lustre@effect:effect(WCI)}),
    fun((WCH) -> lustre@vdom@vnode:element(WCI))
) -> app(WCG, WCH, WCI).
...
-spec start(app(WCP, WCQ, WCR), WCP) -> simulation(WCQ, WCR).
start(App, Args) ->
...
-spec message(simulation(WCX, WCY), WCY) -> simulation(WCX, WCY).
...
-spec problem(simulation(WED, WEE), binary(), binary()) -> simulation(WED, WEE).
...
-spec model(simulation(WEJ, any())) -> WEJ.
model(Simulation) ->
    erlang:element(5, Simulation).
...
-spec view(simulation(any(), WEO)) -> lustre@vdom@vnode:element(WEO).
view(Simulation) ->
    erlang:element(6, Simulation).
...
-spec history(simulation(any(), WET)) -> list(event(WET)).
history(Simulation) ->
...
-spec click(simulation(WDK, WDL), lustre@dev@query:'query'()) -> simulation(WDK, WDL).
...
-spec input(simulation(WDQ, WDR), lustre@dev@query:'query'(), binary()) -> simulation(WDQ, WDR).
