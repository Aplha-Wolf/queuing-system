...
-spec on_attribute_change(binary(), fun((binary()) -> {ok, UPU} | {error, nil})) -> lustre@runtime@app:option(UPU).
...
-spec on_property_change(binary(), gleam@dynamic@decode:decoder(UPY)) -> lustre@runtime@app:option(UPY).
...
-spec on_context_change(binary(), gleam@dynamic@decode:decoder(UQB)) -> lustre@runtime@app:option(UQB).
...
-spec on_form_autofill(fun((binary()) -> UQG)) -> lustre@runtime@app:option(UQG).
...
-spec on_form_reset(UQI) -> lustre@runtime@app:option(UQI).
...
-spec on_form_restore(fun((binary()) -> UQK)) -> lustre@runtime@app:option(UQK).
...
-spec on_connect(UQS) -> lustre@runtime@app:option(UQS).
on_connect(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_adopt(UQU) -> lustre@runtime@app:option(UQU).
on_adopt(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_disconnect(UQW) -> lustre@runtime@app:option(UQW).
...
-spec default_slot(
    list(lustre@vdom@vattr:attribute(UQY)),
    list(lustre@vdom@vnode:element(UQY))
) -> lustre@vdom@vnode:element(UQY).
...
-spec named_slot(
    binary(),
    list(lustre@vdom@vattr:attribute(URE)),
    list(lustre@vdom@vnode:element(URE))
) -> lustre@vdom@vnode:element(URE).
...
-spec prerender(
    lustre@runtime@app:app(nil, any(), USE),
    binary(),
    list(lustre@vdom@vattr:attribute(USE)),
    list(lustre@vdom@vnode:element(USE))
) -> lustre@vdom@vnode:element(USE).
