...
-type client_message(RWB) :: {mount,
        ...
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(RWB)), lustre@vdom@vnode:element(RWB))} |
    {reconcile,
        integer(),
        lustre@vdom@patch:patch(RWB),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(RWB)), lustre@vdom@vnode:element(RWB))} |
...
-spec mount_to_json(
    ...
    list(binary()),
    gleam@dict:dict(binary(), gleam@json:json()),
    lustre@vdom@vnode:element(RXB),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(RXB)), lustre@vdom@vnode:element(RXB))
) -> gleam@json:json().
mount_to_json(
    Kind,
    Open_shadow_root,
...
-spec reconcile_to_json(
    integer(),
    lustre@vdom@patch:patch(RXE),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(RXE)), lustre@vdom@vnode:element(RXE))
) -> gleam@json:json().
...
-spec mount(
    ...
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(RWH)), lustre@vdom@vnode:element(RWH))
) -> client_message(RWH).
mount(
    Open_shadow_root,
    Will_adopt_styles,
...
?DOC(false).
-spec reconcile(
    lustre@vdom@patch:patch(RWL),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(RWL)), lustre@vdom@vnode:element(RWL))
) -> client_message(RWL).
