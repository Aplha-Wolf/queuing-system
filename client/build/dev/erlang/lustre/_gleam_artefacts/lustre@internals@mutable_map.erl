...
-type mutable_map(PTI, PTJ) :: any() | {gleam_phantom, PTI, PTJ}.
...
?DOC(false).
-spec unsafe_get(mutable_map(PTO, PTP), PTO) -> PTP.
unsafe_get(Map, Key) ->
    maps:get(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 34).
?DOC(false).
-spec get_or_compute(mutable_map(PTW, PTX), PTW, fun(() -> PTX)) -> PTX.
...
?DOC(false).
-spec has_key(mutable_map(PUG, any()), PUG) -> boolean().
has_key(Map, Key) ->
    maps:is_key(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 61).
?DOC(false).
-spec insert(mutable_map(PUO, PUP), PUO, PUP) -> mutable_map(PUO, PUP).
insert(Map, Key, Value) ->
    maps:put(Key, Value, Map).

-file("src/lustre/internals/mutable_map.gleam", 79).
?DOC(false).
-spec delete(mutable_map(PVA, PVB), PVA) -> mutable_map(PVA, PVB).
delete(Map, Key) ->
    maps:remove(Key, Map).
