...
-type patch(RQV) :: {patch,
        integer(),
        integer(),
        list(change(RQV)),
        list(patch(RQV))}.

-type change(RQW) :: {replace_text, integer(), binary()} |
    {replace_inner_html, integer(), binary()} |
    {update,
        integer(),
        list(lustre@vdom@vattr:attribute(RQW)),
        list(lustre@vdom@vattr:attribute(RQW))} |
    {move, integer(), binary(), integer()} |
    {replace, integer(), integer(), lustre@vdom@vnode:element(RQW)} |
    {remove, integer(), integer()} |
    {insert, integer(), list(lustre@vdom@vnode:element(RQW)), integer()}.

-file("src/lustre/vdom/patch.gleam", 66).
?DOC(false).
-spec new(integer(), integer(), list(change(RQX)), list(patch(RQX))) -> patch(RQX).
...
?DOC(false).
-spec add_child(patch(RSA), patch(RSA)) -> patch(RSA).
...
-spec replace_to_json(
    ...
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(RUR)), lustre@vdom@vnode:element(RUR))
) -> gleam@json:json().
...
-spec insert_to_json(
    ...
    integer(),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(RUV)), lustre@vdom@vnode:element(RUV))
) -> gleam@json:json().
...
-spec change_to_json(
    change(RSH),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(RSH)), lustre@vdom@vnode:element(RSH))
) -> gleam@json:json().
...
-spec to_json(
    patch(RSE),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(RSE)), lustre@vdom@vnode:element(RSE))
) -> gleam@json:json().
to_json(Patch, Memos) ->
...
?DOC(false).
-spec update(
    list(lustre@vdom@vattr:attribute(RRH)),
    list(lustre@vdom@vattr:attribute(RRH))
) -> change(RRH).
...
?DOC(false).
-spec replace(integer(), lustre@vdom@vnode:element(RRR)) -> change(RRR).
replace(Index, With) ->
    {replace, 5, Index, With}.

-file("src/lustre/vdom/patch.gleam", 116).
?DOC(false).
-spec insert(list(lustre@vdom@vnode:element(RRU)), integer()) -> change(RRU).
