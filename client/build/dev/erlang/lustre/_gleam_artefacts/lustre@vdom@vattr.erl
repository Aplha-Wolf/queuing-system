...
-type attribute(ONX) :: {attribute, integer(), binary(), binary()} |
    {property, integer(), binary(), gleam@json:json()} |
    {event,
        integer(),
        binary(),
        gleam@dynamic@decode:decoder(handler(ONX)),
        list(binary()),
        event_behaviour(),
        event_behaviour(),
        integer(),
...
-type handler(ONY) :: {handler, boolean(), boolean(), ONY}.
...
?DOC(false).
-spec merge(list(attribute(OON)), list(attribute(OON))) -> list(attribute(OON)).
...
?DOC(false).
-spec compare(attribute(OOU), attribute(OOU)) -> gleam@order:order().
compare(A, B) ->
    gleam@string:compare(erlang:element(3, A), erlang:element(3, B)).

-file("src/lustre/vdom/vattr.gleam", 91).
?DOC(false).
-spec prepare(list(attribute(OOI))) -> list(attribute(OOI)).
prepare(Attributes) ->
    case Attributes of
        [] ->
            Attributes;
...
-spec event(
    binary(),
    gleam@dynamic@decode:decoder(handler(OOD)),
    list(binary()),
    event_behaviour(),
    event_behaviour(),
    integer(),
