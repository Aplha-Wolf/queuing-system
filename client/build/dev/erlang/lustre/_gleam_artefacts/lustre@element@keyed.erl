...
-spec do_extract_keyed_children(
    list({binary(), lustre@vdom@vnode:element(WMU)}),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(WMU)),
    list(lustre@vdom@vnode:element(WMU))
) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(WMU)),
    list(lustre@vdom@vnode:element(WMU))}.
...
-spec extract_keyed_children(list({binary(), lustre@vdom@vnode:element(WMM)})) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(WMM)),
...
-spec element(
    ...
    list({binary(), lustre@vdom@vnode:element(WKS)})
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(WKY)),
    list({binary(), lustre@vdom@vnode:element(WKY)})
) -> lustre@vdom@vnode:element(WKY).
...
-spec fragment(list({binary(), lustre@vdom@vnode:element(WLE)})) -> lustre@vdom@vnode:element(WLE).
fragment(Children) ->
    {Keyed_children, Children@1} = extract_keyed_children(Children),
...
-spec ul(
    list(lustre@vdom@vattr:attribute(WLI)),
    list({binary(), lustre@vdom@vnode:element(WLI)})
) -> lustre@vdom@vnode:element(WLI).
ul(Attributes, Children) ->
    element(<<"ul"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 145).
-spec ol(
    list(lustre@vdom@vattr:attribute(WLO)),
    list({binary(), lustre@vdom@vnode:element(WLO)})
) -> lustre@vdom@vnode:element(WLO).
ol(Attributes, Children) ->
    element(<<"ol"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 152).
-spec 'div'(
    list(lustre@vdom@vattr:attribute(WLU)),
    list({binary(), lustre@vdom@vnode:element(WLU)})
) -> lustre@vdom@vnode:element(WLU).
'div'(Attributes, Children) ->
    element(<<"div"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 159).
-spec tbody(
    list(lustre@vdom@vattr:attribute(WMA)),
    list({binary(), lustre@vdom@vnode:element(WMA)})
) -> lustre@vdom@vnode:element(WMA).
tbody(Attributes, Children) ->
    element(<<"tbody"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 166).
-spec dl(
    list(lustre@vdom@vattr:attribute(WMG)),
    list({binary(), lustre@vdom@vnode:element(WMG)})
) -> lustre@vdom@vnode:element(WMG).
