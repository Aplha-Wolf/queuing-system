...
-spec on(binary(), gleam@dynamic@decode:decoder(XLT)) -> lustre@vdom@vattr:attribute(XLT).
on(Name, Handler) ->
...
-spec advanced(
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(XLW))
) -> lustre@vdom@vattr:attribute(XLW).
...
-spec handler(XMA, boolean(), boolean()) -> lustre@vdom@vattr:handler(XMA).
...
-spec prevent_default(lustre@vdom@vattr:attribute(XMC)) -> lustre@vdom@vattr:attribute(XMC).
...
-spec stop_propagation(lustre@vdom@vattr:attribute(XMF)) -> lustre@vdom@vattr:attribute(XMF).
...
-spec debounce(lustre@vdom@vattr:attribute(XMI), integer()) -> lustre@vdom@vattr:attribute(XMI).
...
-spec throttle(lustre@vdom@vattr:attribute(XML), integer()) -> lustre@vdom@vattr:attribute(XML).
...
-spec on_keypress(fun((binary()) -> XNC)) -> lustre@vdom@vattr:attribute(XNC).
on_keypress(Msg) ->
    on(
        <<"keypress"/utf8>>,
        begin
...
-spec on_keydown(fun((binary()) -> XNE)) -> lustre@vdom@vattr:attribute(XNE).
on_keydown(Msg) ->
    on(
        <<"keydown"/utf8>>,
        begin
...
-spec on_keyup(fun((binary()) -> XNG)) -> lustre@vdom@vattr:attribute(XNG).
on_keyup(Msg) ->
    on(
        <<"keyup"/utf8>>,
        begin
...
-spec on_input(fun((binary()) -> XNI)) -> lustre@vdom@vattr:attribute(XNI).
on_input(Msg) ->
    on(
        <<"input"/utf8>>,
        begin
...
-spec on_change(fun((binary()) -> XNK)) -> lustre@vdom@vattr:attribute(XNK).
on_change(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_check(fun((boolean()) -> XNM)) -> lustre@vdom@vattr:attribute(XNM).
on_check(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_submit(fun((list({binary(), binary()})) -> XNP)) -> lustre@vdom@vattr:attribute(XNP).
on_submit(Msg) ->
    _pipe@2 = on(
        <<"submit"/utf8>>,
        begin
...
-spec on_focus(XNT) -> lustre@vdom@vattr:attribute(XNT).
on_focus(Msg) ->
    on(<<"focus"/utf8>>, gleam@dynamic@decode:success(Msg)).

-file("src/lustre/event.gleam", 352).
-spec on_blur(XNV) -> lustre@vdom@vattr:attribute(XNV).
on_blur(Msg) ->
