...
-type state(TEM, TEN) :: {state,
        gleam@erlang@process:subject(message(TEN)),
        gleam@erlang@process:selector(message(TEN)),
        gleam@erlang@process:selector(message(TEN)),
        TEM,
        fun((TEM, TEN) -> {TEM, lustre@effect:effect(TEN)}),
        fun((TEM) -> lustre@vdom@vnode:element(TEN)),
        config(TEN),
        lustre@vdom@vnode:element(TEN),
        lustre@vdom@cache:cache(TEN),
        gleam@dict:dict(binary(), gleam@json:json()),
        gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(TEN)), gleam@erlang@process:monitor()),
        gleam@set:set(fun((lustre@runtime@transport:client_message(TEN)) -> nil))}.

-type config(TEO) :: {config,
        boolean(),
        boolean(),
        gleam@dict:dict(binary(), fun((binary()) -> {ok, TEO} | {error, nil})),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(TEO)),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(TEO)),
        gleam@option:option(TEO),
        gleam@option:option(TEO)}.

-type message(TEP) :: {client_dispatched_message,
        lustre@runtime@transport:server_message()} |
    {client_registered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(TEP))} |
    {client_deregistered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(TEP))} |
    {client_registered_callback,
        fun((lustre@runtime@transport:client_message(TEP)) -> nil)} |
    {client_deregistered_callback,
        fun((lustre@runtime@transport:client_message(TEP)) -> nil)} |
    {effect_added_selector, gleam@erlang@process:selector(message(TEP))} |
    {effect_dispatched_message, TEP} |
...
-spec handle_attribute_change(
    gleam@dict:dict(binary(), fun((binary()) -> {ok, TFY} | {error, nil})),
    binary(),
    binary()
) -> {ok, TFY} | {error, nil}.
...
-spec handle_property_change(
    gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(TGF)),
    binary(),
...
?DOC(false).
-spec handle_effect(
    gleam@erlang@process:subject(message(TGL)),
    lustre@effect:effect(TGL)
) -> nil.
...
?DOC(false).
-spec broadcast(
    gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(TGP)), gleam@erlang@process:monitor()),
    gleam@set:set(fun((lustre@runtime@transport:client_message(TGP)) -> nil)),
    lustre@runtime@transport:client_message(TGP)
) -> nil.
...
?DOC(false).
-spec loop(state(TFI, TFJ), message(TFJ)) -> gleam@otp@actor:next(state(TFI, TFJ), message(TFJ)).
loop(State, Message) ->
    case Message of
...
?DOC(false).
-spec start(
    gleam@option:option(gleam@erlang@process:name(message(TET))),
    fun((TEX) -> {TEY, lustre@effect:effect(TET)}),
    fun((TEY, TET) -> {TEY, lustre@effect:effect(TET)}),
    fun((TEY) -> lustre@vdom@vnode:element(TET)),
    config(TET),
    TEX
) -> {ok, gleam@otp@actor:started(gleam@erlang@process:subject(message(TET)))} |
