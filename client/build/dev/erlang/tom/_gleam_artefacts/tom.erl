...
-spec push_key({ok, ADED} | {error, get_error()}, binary()) -> {ok, ADED} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {ADGI, list(binary())}} |
        {error, parse_error()})
) -> {ok, {ADGI, list(binary())}} | {error, parse_error()}.
...
-spec do(
    {ok, {ADGT, list(binary())}} | {error, parse_error()},
    fun((ADGT, list(binary())) -> {ok, ADGW} | {error, parse_error()})
) -> {ok, ADGW} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {ADHB, list(binary())}} |
        {error, parse_error()})
) -> {ok, {ADHB, list(binary())}} | {error, parse_error()}.
