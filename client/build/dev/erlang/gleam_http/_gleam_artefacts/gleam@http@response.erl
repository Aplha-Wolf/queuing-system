...
-type response(GCG) :: {response, integer(), list({binary(), binary()}), GCG}.
...
-spec set_header(response(GCV), binary(), binary()) -> response(GCV).
...
-spec prepend_header(response(GCY), binary(), binary()) -> response(GCY).
...
-spec set_body(response(any()), GDD) -> response(GDD).
...
-spec try_map(response(GCH), fun((GCH) -> {ok, GCJ} | {error, GCK})) -> {ok,
        response(GCJ)} |
    {error, GCK}.
...
-spec map(response(GDF), fun((GDF) -> GDH)) -> response(GDH).
...
-spec expire_cookie(response(GDQ), binary(), gleam@http@cookie:attributes()) -> response(GDQ).
