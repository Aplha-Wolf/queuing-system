...
-spec set_header(request(FSM), binary(), binary()) -> request(FSM).
...
-spec prepend_header(request(FSP), binary(), binary()) -> request(FSP).
...
-spec set_body(request(any()), FSU) -> request(FSU).
set_body(Req, Body) ->
    {request,
...
-spec map(request(FSW), fun((FSW) -> FSY)) -> request(FSY).
...
-spec set_query(request(FTI), list({binary(), binary()})) -> request(FTI).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(FTM), gleam@http:method()) -> request(FTM).
...
-spec set_scheme(request(FTT), gleam@http:scheme()) -> request(FTT).
...
-spec set_host(request(FTW), binary()) -> request(FTW).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(FTZ), integer()) -> request(FTZ).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(FUC), binary()) -> request(FUC).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(FUF), binary(), binary()) -> request(FUF).
...
-spec remove_cookie(request(FUL), binary()) -> request(FUL).
