...
-opaque subject(EWK) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(EWK)}.

-type name(EWL) :: any() | {gleam_phantom, EWL}.

-type do_not_leak() :: any().

-type selector(EWM) :: any() | {gleam_phantom, EWM}.
...
-spec 'receive'(subject(EXK), integer()) -> {ok, EXK} | {error, nil}.
...
-spec receive_forever(subject(EXS)) -> EXS.
...
-spec selector_receive(selector(EXW), integer()) -> {ok, EXW} | {error, nil}.
...
-spec selector_receive_forever(selector(EYA)) -> EYA.
...
-spec map_selector(selector(EYC), fun((EYC) -> EYE)) -> selector(EYE).
map_selector(A, B) ->
...
-spec merge_selector(selector(EYG), selector(EYG)) -> selector(EYG).
merge_selector(A, B) ->
...
-spec select_map(selector(EYR), subject(EYT), fun((EYT) -> EYR)) -> selector(EYR).
...
-spec select(selector(EYN), subject(EYN)) -> selector(EYN).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> EZB)
) -> selector(EZB).
...
-spec select_other(selector(EZF), fun((gleam@dynamic:dynamic_()) -> EZF)) -> selector(EZF).
...
-spec deselect(selector(EYW), subject(any())) -> selector(EYW).
...
-spec select_specific_monitor(selector(EZR), monitor(), fun((down()) -> EZR)) -> selector(EZR).
...
-spec select_monitors(selector(EZU), fun((down()) -> EZU)) -> selector(EZU).
...
-spec select_trapped_exits(selector(EYK), fun((exit_message()) -> EYK)) -> selector(EYK).
...
-spec deselect_specific_monitor(selector(EZX), monitor()) -> selector(EZX).
...
-spec send_after(subject(FAT), integer(), FAT) -> timer().
...
-spec perform_call(
    subject(FAA),
    fun((subject(FAC)) -> FAA),
    fun((selector(FAC)) -> {ok, FAC} | {error, nil})
) -> FAC.
...
-spec call(subject(FAH), integer(), fun((subject(FAJ)) -> FAH)) -> FAJ.
...
-spec call_forever(subject(FAL), fun((subject(FAN)) -> FAL)) -> FAN.
