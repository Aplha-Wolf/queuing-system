...
-type message(GVG) :: {message, GVG} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(GVH, GVI) :: {continue,
        GVH,
        gleam@option:option(gleam@erlang@process:selector(GVI))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(GVJ, GVK) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        GVJ,
        gleam@erlang@process:selector(message(GVK)),
        gleam@otp@system:debug_state(),
        fun((GVJ, GVK) -> next(GVJ, GVK))}.

-type started(GVL) :: {started, gleam@erlang@process:pid_(), GVL}.

-opaque initialised(GVM, GVN, GVO) :: {initialised,
        GVM,
        gleam@option:option(gleam@erlang@process:selector(GVN)),
        GVO}.

-opaque builder(GVP, GVQ, GVR) :: {builder,
        fun((gleam@erlang@process:subject(GVQ)) -> {ok,
                initialised(GVP, GVQ, GVR)} |
            {error, binary()}),
        integer(),
        fun((GVP, GVQ) -> next(GVP, GVQ)),
        gleam@option:option(gleam@erlang@process:name(GVQ))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(GVS) :: {ack, {ok, GVS} | {error, binary()}} |
...
-spec continue(GVX) -> next(GVX, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(GWJ, GWK), gleam@erlang@process:selector(GWK)) -> next(GWJ, GWK).
...
-spec initialised(GWQ) -> initialised(GWQ, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(GWV, any(), GWX),
    gleam@erlang@process:selector(GXB)
) -> initialised(GWV, GXB, GWX).
...
-spec returning(initialised(GXG, GXH, any()), GXM) -> initialised(GXG, GXH, GXM).
...
-spec new(GXQ) -> builder(GXQ, GXR, gleam@erlang@process:subject(GXR)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(GXW)) -> {ok, initialised(GXY, GXW, GXZ)} |
        {error, binary()})
) -> builder(GXY, GXW, GXZ).
...
-spec on_message(builder(GYI, GYJ, GYK), fun((GYI, GYJ) -> next(GYI, GYJ))) -> builder(GYI, GYJ, GYK).
...
-spec named(builder(GYT, GYU, GYV), gleam@erlang@process:name(GYU)) -> builder(GYT, GYU, GYV).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(GZI))) -> gleam@erlang@process:selector(message(GZI)).
...
-spec receive_message(self(any(), GZE)) -> message(GZE).
...
-spec initialise_actor(
    builder(any(), any(), HAB),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, HAB} | {error, binary()})
...
-spec start(builder(any(), any(), HAO)) -> {ok, started(HAO)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(HAV), HAV) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(HAX),
    integer(),
    fun((gleam@erlang@process:subject(HAZ)) -> HAX)
) -> HAZ.
