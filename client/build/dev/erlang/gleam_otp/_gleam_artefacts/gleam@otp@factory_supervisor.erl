...
-opaque supervisor(HQZ, HRA) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(HQZ, HRA))}.

-type message(HRB, HRC) :: any() | {gleam_phantom, HRB, HRC}.

-opaque builder(HRD, HRE) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((HRD) -> {ok, gleam@otp@actor:started(HRE)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(HRD, HRE)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(HRF, HRG) :: {local,
        gleam@erlang@process:name(message(HRF, HRG))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(HRH) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, HRH}.
...
-type erlang_child_spec_property(HRI, HRJ) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((HRI) -> {ok, gleam@otp@actor:started(HRJ)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(HRK, HRL))) -> supervisor(HRK, HRL).
get_by_name(Name) ->
...
-spec named(builder(HSB, HSC), gleam@erlang@process:name(message(HSB, HSC))) -> builder(HSB, HSC).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(HSK, HSL), integer(), integer()) -> builder(HSK, HSL).
...
-spec timeout(builder(HSQ, HSR), integer()) -> builder(HSQ, HSR).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(HSW, HSX), gleam@otp@supervision:restart()) -> builder(HSW, HSX).
...
-spec start(builder(HTC, HTD)) -> {ok,
        gleam@otp@actor:started(supervisor(HTC, HTD))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(HUB, HUC)) -> gleam@otp@supervision:child_specification(supervisor(HUB, HUC)).
supervised(Builder) ->
...
-spec start_child(supervisor(HUI, HUJ), HUI) -> {ok,
        gleam@otp@actor:started(HUJ)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((HVG) -> {ok, gleam@otp@actor:started(HVH)} |
        {error, gleam@otp@actor:start_error()}),
    HVG
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), HVH, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((HRR) -> {ok, gleam@otp@actor:started(HRS)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(HRR, HRS).
...
-spec supervisor_child(
    fun((HRW) -> {ok, gleam@otp@actor:started(HRX)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(HRW, HRX).
