...
-type child_specification(HOL) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(HOL)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(HOM)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(HOM).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(HOR)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(HOR).
supervisor(Start) ->
...
-spec significant(child_specification(HOW), boolean()) -> child_specification(HOW).
...
-spec timeout(child_specification(HOZ), integer()) -> child_specification(HOZ).
timeout(Child, Ms) ->
...
-spec restart(child_specification(HPC), restart()) -> child_specification(HPC).
...
-spec map_data(child_specification(HPF), fun((HPF) -> HPH)) -> child_specification(HPH).
