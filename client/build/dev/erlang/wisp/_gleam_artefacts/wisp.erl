...
-spec method_override(gleam@http@request:request(AGBG)) -> gleam@http@request:request(AGBG).
...
-spec sort_keys(list({binary(), AGCX})) -> list({binary(), AGCX}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_500({ok, AGBR} | {error, any()}) -> {ok, AGBR} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
...
-spec fn_with_bad_request_error(
    fun((AGCF) -> {ok, AGCG} | {error, any()}),
    binary()
) -> fun((AGCF) -> {ok, AGCG} | {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((AGBZ, bitstring()) -> {ok, AGBZ} |
        {error, gleam@http@response:response(body())}),
    AGBZ
) -> {ok, {gleam@option:option(buffered_reader()), integer(), AGBZ}} |
    {error, gleam@http@response:response(body())}.
