...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(EEB)) -> {ok, EEB} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(EDV)) -> {ok, EDV} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(EDR)) -> {ok, EDR} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(EEH), fun((EEH) -> json())) -> json().
...
-spec array(list(EEL), fun((EEL) -> json())) -> json().
