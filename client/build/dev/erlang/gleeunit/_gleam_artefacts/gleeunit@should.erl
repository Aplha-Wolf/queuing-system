...
-spec be_ok({ok, KBA} | {error, any()}) -> KBA.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, KBF}) -> KBF.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(KBI)) -> KBI.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
