...
-spec string(
    ...
    fun((fun((glint:flags()) -> {ok, binary()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AHMP))
) -> glint:command(AHMP).
...
-spec flag(
    ...
    list(binary()),
    lustre_dev_tools@project:project(),
    fun((binary()) -> glint:flag(AHNG)),
    fun((gleam@dict:dict(binary(), tom:toml()), list(binary())) -> {ok, AHNG} |
        {error, tom:get_error()}),
    binary(),
    fun((fun((glint:flags()) -> {ok, AHNG} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AHNP))
) -> glint:command(AHNP).
...
-spec bool(
    ...
    fun((fun((glint:flags()) -> {ok, boolean()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AHMJ))
) -> glint:command(AHMJ).
...
-spec string_list(
    ...
    fun((fun((glint:flags()) -> {ok, list(binary())} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AHMW))
) -> glint:command(AHMW).
...
-spec int(
    ...
    fun((fun((glint:flags()) -> {ok, integer()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AHNC))
) -> glint:command(AHNC).
