...
-opaque glint(KMB) :: {glint, config(), command_node(KMB)}.

-type args_count() :: {eq_args, integer()} | {min_args, integer()}.

-opaque command(KMC) :: {command,
        fun((named_args(), list(binary()), flags()) -> KMC),
        flags(),
        binary(),
        gleam@option:option(args_count()),
        list(binary())}.

-type internal_command(KMD) :: {internal_command,
        fun((named_args(), list(binary()), flags()) -> KMD),
        flags(),
        gleam@option:option(args_count()),
        list(binary())}.
...
-type command_node(KME) :: {command_node,
        gleam@option:option(internal_command(KME)),
        gleam@dict:dict(binary(), command_node(KME)),
        flags(),
        binary()}.

-type out(KMF) :: {out, KMF} | {help, binary()}.
...
-opaque flag(KMG) :: {flag,
        binary(),
        binary(),
        fun((binary()) -> {ok, KMG} | {error, snag:snag()}),
        fun((flag_internals(KMG)) -> value()),
        fun((flags(), binary()) -> {ok, KMG} | {error, snag:snag()}),
        gleam@option:option(KMG)}.

-type flag_internals(KMH) :: {flag_internals,
        gleam@option:option(KMH),
        fun((binary()) -> {ok, KMH} | {error, snag:snag()})}.
...
-spec pretty_help(glint(KMO), pretty_help()) -> glint(KMO).
...
-spec with_name(glint(KMR), binary()) -> glint(KMR).
...
-spec without_exit(glint(KMU)) -> glint(KMU).
without_exit(Glint) ->
    {glint,
        begin
            _record = erlang:element(2, Glint),
...
-spec as_module(glint(KMX)) -> glint(KMX).
as_module(Glint) ->
    {glint,
        begin
            _record = erlang:element(2, Glint),
...
-spec with_indent_width(glint(KNA), integer()) -> glint(KNA).
...
-spec with_max_output_width(glint(KND), integer()) -> glint(KND).
...
-spec with_min_first_column_width(glint(KNG), integer()) -> glint(KNG).
...
-spec with_column_gap(glint(KNJ), integer()) -> glint(KNJ).
...
-spec global_help(glint(KNS), binary()) -> glint(KNS).
...
-spec map_command(command(KOH), fun((KOH) -> KOJ)) -> command(KOJ).
...
-spec command_help(binary(), fun(() -> command(KOL))) -> command(KOL).
command_help(Desc, F) ->
    _record = F(),
    {command,
        erlang:element(2, _record),
...
-spec unnamed_args(args_count(), fun(() -> command(KOO))) -> command(KOO).
...
-spec named_arg(
    binary(),
    fun((fun((named_args()) -> binary())) -> command(KOR))
) -> command(KOR).
named_arg(Name, F) ->
    Cmd = begin
        F(
...
-spec new_builder(
    binary(),
    fun((flag_internals(KRF)) -> value()),
    fun((flags(), binary()) -> {ok, KRF} | {error, snag:snag()}),
    fun((binary()) -> {ok, KRF} | {error, snag:snag()})
) -> flag(KRF).
...
-spec wrap_with_constraint(
    fun((binary()) -> {ok, KRR} | {error, snag:snag()}),
    fun((KRR) -> {ok, KRR} | {error, snag:snag()})
) -> fun((binary()) -> {ok, KRR} | {error, snag:snag()}).
...
-spec flag_constraint(flag(KRN), fun((KRN) -> {ok, KRN} | {error, snag:snag()})) -> flag(KRN).
...
-spec flag_help(flag(KSG), binary()) -> flag(KSG).
...
-spec flag_default(flag(KSJ), KSJ) -> flag(KSJ).
...
-spec flag(
    flag(KOU),
    fun((fun((flags()) -> {ok, KOU} | {error, snag:snag()})) -> command(KOX))
) -> command(KOX).
flag(Flag, F) ->
    Cmd = F(
        fun(_capture) ->
...
-spec fold(flags(), KSM, fun((KSM, binary(), flag_entry()) -> KSM)) -> KSM.
fold(Flags, Acc, F) ->
...
-spec command(fun((named_args(), list(binary()), flags()) -> KOE)) -> command(KOE).
command(Runner) ->
...
-spec get_value(
    flags(),
    binary(),
    fun((flag_entry()) -> {ok, KSZ} | {error, snag:snag()})
) -> {ok, KSZ} | {error, snag:snag()}.
...
-spec get_flag(flags(), flag(KTC)) -> {ok, KTC} | {error, snag:snag()}.
get_flag(Flags, Flag) ->
...
-spec do_update_at(
    command_node(KTV),
    list(binary()),
    fun((command_node(KTV)) -> command_node(KTV))
) -> command_node(KTV).
...
-spec update_at(
    glint(KTP),
    list(binary()),
    fun((command_node(KTP)) -> command_node(KTP))
) -> glint(KTP).
...
-spec path_help(glint(KNO), list(binary()), binary()) -> glint(KNO).
...
-spec add(glint(KNV), list(binary()), command(KNV)) -> glint(KNV).
...
-spec group_flag(glint(KPA), list(binary()), flag(any())) -> glint(KPA).
...
?DOC(false).
-spec execute(glint(KPG), list(binary())) -> {ok, out(KPG)} | {error, binary()}.
execute(Glint, Args) ->
    Help_flag = <<"--"/utf8,
        (erlang:element(
            2,
...
-spec run_and_handle(glint(KQF), list(binary()), fun((KQF) -> any())) -> nil.
