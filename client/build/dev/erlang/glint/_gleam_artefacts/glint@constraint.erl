...
-spec one_of(list(KFR)) -> fun((KFR) -> {ok, KFR} | {error, snag:snag()}).
one_of(Allowed) ->
...
-spec none_of(list(KFU)) -> fun((KFU) -> {ok, KFU} | {error, snag:snag()}).
none_of(Disallowed) ->
...
-spec each(fun((KFX) -> {ok, KFX} | {error, snag:snag()})) -> fun((list(KFX)) -> {ok,
        list(KFX)} |
    {error, snag:snag()}).
each(Constraint) ->
