...
-define(FILEPATH, "src/rsvp.gleam").
-export([expect_ok_response/1, expect_text/1, expect_any_response/1, send/2, send_bits/2, simulate/3, expect_json/2, parse_relative_uri/1, get/2, post/3, put/3, patch/3, delete/3]).
...
-opaque handler(ALPP) :: {handler,
        fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> ALPP)}.

-file("src/rsvp.gleam", 190).
?DOC(
...
-spec expect_ok_response(
    fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> ALQN)
) -> handler(ALQN).
...
-file("src/rsvp.gleam", 118).
-spec expect_json_response(
    fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> ALPZ)
) -> handler(ALPZ).
...
-file("src/rsvp.gleam", 163).
-spec expect_text_response(
    fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> ALQI)
) -> handler(ALQI).
...
-spec expect_text(fun(({ok, binary()} | {error, error()}) -> ALQD)) -> handler(ALQD).
expect_text(Handler) ->
...
-spec expect_any_response(
    fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> ALQS)
) -> handler(ALQS).
expect_any_response(Handler) ->
    {handler, Handler}.

-file("src/rsvp.gleam", 403).
-spec do_send(gleam@http@request:request(binary()), handler(ALRO)) -> lustre@effect:effect(ALRO).
do_send(Request, Handler) ->
    lustre@effect:from(
        fun(Dispatch) ->
            proc_lib:spawn_link(fun() -> _pipe = gleam@httpc:send(Request),
...
-spec send(gleam@http@request:request(binary()), handler(ALRK)) -> lustre@effect:effect(ALRK).
send(Request, Handler) ->
    do_send(Request, Handler).

-file("src/rsvp.gleam", 460).
-spec do_send_bits(gleam@http@request:request(bitstring()), handler(ALRW)) -> lustre@effect:effect(ALRW).
do_send_bits(Request, Handler) ->
    lustre@effect:from(
        fun(Dispatch) ->
            proc_lib:spawn_link(fun() -> _pipe = gleam@httpc:send_bits(Request),
                    _pipe@1 = gleam@result:map_error(
                        _pipe,
                        fun(Error) -> case Error of
                                invalid_utf8_response ->
                    ...
                    _pipe@3 = (erlang:element(2, Handler))(_pipe@2),
                    Dispatch(_pipe@3) end),
            nil
        end
    ).

-file("src/rsvp.gleam", 452).
?DOC(
    " Send a [`Request`](https://hexdocs.pm/gleam_http/gleam/http/request.html#Request)\n"
...
-spec send_bits(gleam@http@request:request(bitstring()), handler(ALRS)) -> lustre@effect:effect(ALRS).
send_bits(Request, Handler) ->
    do_send_bits(Request, Handler).

-file("src/rsvp.gleam", 519).
?DOC(
...
-spec simulate(
    lustre@dev@simulate:simulation(ALRZ, ALSA),
...
-file("src/rsvp.gleam", 529).
-spec reject(error(), handler(ALSH)) -> lustre@effect:effect(ALSH).
reject(Err, Handler) ->
    lustre@effect:from(fun(Dispatch) -> _pipe = {error, Err},
            _pipe@1 = (erlang:element(2, Handler))(_pipe),
            Dispatch(_pipe@1) end).

-file("src/rsvp.gleam", 537).
-spec decode_json_body(
    gleam@http@response:response(binary()),
    gleam@dynamic@decode:decoder(ALSL)
) -> {ok, ALSL} | {error, error()}.
...
-spec expect_json(
    gleam@dynamic@decode:decoder(ALPQ),
    fun(({ok, ALPQ} | {error, error()}) -> ALPU)
) -> handler(ALPU).
...
-spec get(binary(), handler(ALQU)) -> lustre@effect:effect(ALQU).
get(Url, Handler) ->
    case to_uri(Url) of
        {ok, Uri} ->
...
-spec post(binary(), gleam@json:json(), handler(ALQX)) -> lustre@effect:effect(ALQX).
...
-spec put(binary(), gleam@json:json(), handler(ALRA)) -> lustre@effect:effect(ALRA).
put(Url, Body, Handler) ->
    case to_uri(Url) of
        {ok, Uri} ->
            case gleam@http@request:from_uri(Uri) of
                {ok, Request} ->
                    _pipe = Request,
                    _pipe@1 = gleam@http@request:set_method(_pipe, put),
                    _pipe@2 = gleam@http@request:set_header(
                        _pipe@1,
                        <<"content-type"/utf8>>,
                        <<"application/json"/utf8>>
...
-spec patch(binary(), gleam@json:json(), handler(ALRD)) -> lustre@effect:effect(ALRD).
patch(Url, Body, Handler) ->
    case to_uri(Url) of
        {ok, Uri} ->
            case gleam@http@request:from_uri(Uri) of
                {ok, Request} ->
                    _pipe = Request,
                    _pipe@1 = gleam@http@request:set_method(_pipe, patch),
                    _pipe@2 = gleam@http@request:set_header(
                        _pipe@1,
                        <<"content-type"/utf8>>,
                        <<"application/json"/utf8>>
...
-spec delete(binary(), gleam@json:json(), handler(ALRG)) -> lustre@effect:effect(ALRG).
delete(Url, Body, Handler) ->
    case to_uri(Url) of
        {ok, Uri} ->
            case gleam@http@request:from_uri(Uri) of
                {ok, Request} ->
