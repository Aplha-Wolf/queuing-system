...
-type action(ISN) :: stop | {continue, ISN, fun(() -> action(ISN))}.

-opaque yielder(ISO) :: {yielder, fun(() -> action(ISO))}.

-type step(ISP, ISQ) :: {next, ISP, ISQ} | done.

-type chunk(ISR, ISS) :: {another_by,
        list(ISR),
        ISS,
        ISR,
        fun(() -> action(ISR))} |
...
-type sized_chunk(IST) :: {another, list(IST), fun(() -> action(IST))} |
    {last, list(IST)} |
    no_more.
...
-spec unfold_loop(ITB, fun((ITB) -> step(ITC, ITB))) -> fun(() -> action(ITC)).
...
-spec unfold(ISW, fun((ISW) -> step(ISX, ISW))) -> yielder(ISX).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> ITG)) -> yielder(ITG).
repeatedly(F) ->
...
-spec from_list(list(ITK)) -> yielder(ITK).
from_list(List) ->
...
-spec transform(yielder(ITU), ITW, fun((ITW, ITU) -> step(ITX, ITW))) -> yielder(ITX).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(IUE)), fun((IUG, IUE) -> IUG), IUG) -> IUG.
...
-spec fold(yielder(IUB), IUD, fun((IUD, IUB) -> IUD)) -> IUD.
...
-spec to_list(yielder(IUJ)) -> list(IUJ).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(IUM)) -> step(IUM, yielder(IUM)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(IUU)), integer()) -> fun(() -> action(IUU)).
...
-spec take(yielder(IUR), integer()) -> yielder(IUR).
...
-spec drop_loop(fun(() -> action(IVA)), integer()) -> action(IVA).
...
-spec map(yielder(IVD), fun((IVD) -> IVF)) -> yielder(IVF).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(IVL), yielder(IVN), fun((IVL, IVN) -> IVP)) -> yielder(IVP).
...
-spec append_loop(fun(() -> action(IWB)), fun(() -> action(IWB))) -> action(IWB).
...
-spec append(yielder(IVX), yielder(IVX)) -> yielder(IVX).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(IWJ)))) -> action(IWJ).
...
-spec flatten(yielder(yielder(IWF))) -> yielder(IWF).
flatten(Yielder) ->
...
-spec concat(list(yielder(IWN))) -> yielder(IWN).
concat(Yielders) ->
...
-spec flat_map(yielder(IWR), fun((IWR) -> yielder(IWT))) -> yielder(IWT).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(IWZ)), fun((IWZ) -> boolean())) -> action(IWZ).
...
-spec filter(yielder(IWW), fun((IWW) -> boolean())) -> yielder(IWW).
...
-spec filter_map(yielder(IXC), fun((IXC) -> {ok, IXE} | {error, any()})) -> yielder(IXE).
...
-spec cycle(yielder(IXQ)) -> yielder(IXQ).
cycle(Yielder) ->
    _pipe = repeat(Yielder),
    flatten(_pipe).

-file("src/gleam/yielder.gleam", 709).
-spec find_loop(fun(() -> action(IXY)), fun((IXY) -> boolean())) -> {ok, IXY} |
    {error, nil}.
...
-spec find(yielder(IXU), fun((IXU) -> boolean())) -> {ok, IXU} | {error, nil}.
...
-spec find_map(yielder(IYC), fun((IYC) -> {ok, IYE} | {error, any()})) -> {ok,
        IYE} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(IYV)), integer()) -> fun(() -> action({IYV,
    integer()})).
...
-spec index(yielder(IYS)) -> yielder({IYS, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(IYY, fun((IYY) -> IYY)) -> yielder(IYY).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(IZD)), fun((IZD) -> boolean())) -> fun(() -> action(IZD)).
...
-spec take_while(yielder(IZA), fun((IZA) -> boolean())) -> yielder(IZA).
...
-spec drop_while_loop(fun(() -> action(IZJ)), fun((IZJ) -> boolean())) -> action(IZJ).
...
-spec drop_while(yielder(IZG), fun((IZG) -> boolean())) -> yielder(IZG).
...
-spec scan_loop(fun(() -> action(IZQ)), fun((IZS, IZQ) -> IZS), IZS) -> fun(() -> action(IZS)).
...
-spec scan(yielder(IZM), IZO, fun((IZO, IZM) -> IZO)) -> yielder(IZO).
...
-spec zip_loop(fun(() -> action(IZZ)), fun(() -> action(JAB))) -> fun(() -> action({IZZ,
    JAB})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(IZU), yielder(IZW)) -> yielder({IZU, IZW}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(JAO)), fun((JAO) -> JAQ), JAQ, list(JAO)) -> chunk(JAO, JAQ).
...
-spec chunk_loop(fun(() -> action(JAJ)), fun((JAJ) -> JAL), JAL, JAJ) -> action(list(JAJ)).
...
-spec chunk(yielder(JAE), fun((JAE) -> any())) -> yielder(list(JAE)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(JBC)), integer(), list(JBC)) -> sized_chunk(JBC).
...
-spec sized_chunk_loop(fun(() -> action(JAY)), integer()) -> fun(() -> action(list(JAY))).
...
-spec sized_chunk(yielder(JAU), integer()) -> yielder(list(JAU)).
...
-spec intersperse_loop(fun(() -> action(JBJ)), JBJ) -> action(JBJ).
...
-spec intersperse(yielder(JBG), JBG) -> yielder(JBG).
...
-spec any_loop(fun(() -> action(JBO)), fun((JBO) -> boolean())) -> boolean().
...
-spec any(yielder(JBM), fun((JBM) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(JBS)), fun((JBS) -> boolean())) -> boolean().
...
-spec all(yielder(JBQ), fun((JBQ) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(JCI) -> fun((gleam@option:option(list(JCI))) -> list(JCI)).
...
-spec group_updater(fun((JCA) -> JCB)) -> fun((gleam@dict:dict(JCB, list(JCA)), JCA) -> gleam@dict:dict(JCB, list(JCA))).
group_updater(F) ->
...
-spec group(yielder(JBU), fun((JBU) -> JBW)) -> gleam@dict:dict(JBW, list(JBU)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(JCM), fun((JCM, JCM) -> JCM)) -> {ok, JCM} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(JCQ)) -> {ok, JCQ} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(JCY) -> yielder(JCY).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(JDE)), fun(() -> action(JDE))) -> action(JDE).
...
-spec interleave(yielder(JDA), yielder(JDA)) -> yielder(JDA).
...
-spec fold_until_loop(
    fun(() -> action(JDM)),
    fun((JDO, JDM) -> gleam@list:continue_or_stop(JDO)),
    JDO
) -> JDO.
...
-spec fold_until(
    yielder(JDI),
    JDK,
    fun((JDK, JDI) -> gleam@list:continue_or_stop(JDK))
) -> JDK.
...
-spec try_fold(yielder(JDQ), JDS, fun((JDS, JDQ) -> {ok, JDS} | {error, JDT})) -> {ok,
        JDS} |
    {error, JDT}.
...
-spec first(yielder(JEG)) -> {ok, JEG} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(JEK), integer()) -> {ok, JEK} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(JES), fun((JES) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(JEV, fun(() -> yielder(JEV))) -> yielder(JEV).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(JEY), JEY) -> yielder(JEY).
