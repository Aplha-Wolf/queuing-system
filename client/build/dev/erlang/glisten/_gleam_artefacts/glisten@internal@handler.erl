...
-type message(MGD) :: {internal, internal_message()} | {user, MGD}.

-type loop_message(MGE) :: {packet, bitstring()} | {custom, MGE}.

-type loop_state(MGF, MGG) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(MGG)),
        glisten@transport:transport(),
        MGF}.

-type connection(MGH) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(MGH))}.

-type next(MGI, MGJ) :: {continue,
        MGI,
        gleam@option:option(gleam@erlang@process:selector(MGJ))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(MGK, MGL) :: {handler,
        glisten@socket:socket(),
        fun((MGK, loop_message(MGL), connection(MGL)) -> next(MGK, loop_message(MGL))),
        fun((connection(MGL)) -> {MGK,
            gleam@option:option(gleam@erlang@process:selector(MGL))}),
        gleam@option:option(fun((MGK) -> nil)),
        glisten@transport:transport()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(MGY) -> next(MGY, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(MHC, MHD), gleam@erlang@process:selector(MHD)) -> next(MHC, MHD).
...
?DOC(false).
-spec start(handler(any(), MHS)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(MHS)))} |
