...
-type message(NAE) :: {packet, bitstring()} | {user, NAE}.
...
-type connection(NAF) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(NAF))}.

-opaque next(NAG, NAH) :: {continue,
        NAG,
        gleam@option:option(gleam@erlang@process:selector(NAH))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(NAI, NAJ) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(NAJ)) -> {NAI,
            gleam@option:option(gleam@erlang@process:selector(NAJ))}),
        fun((NAI, message(NAJ), connection(NAJ)) -> next(NAI, message(NAJ))),
        gleam@option:option(fun((NAI) -> nil)),
        integer(),
        boolean(),
        boolean(),
...
-spec continue(NBJ) -> next(NBJ, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 151).
-spec with_selector(next(NBN, NBO), gleam@erlang@process:selector(NBO)) -> next(NBN, NBO).
...
?DOC(false).
-spec convert_next(next(NCC, NCD)) -> glisten@internal@handler:next(NCC, NCD).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(NCI, NCJ), fun((NCJ) -> NCM)) -> next(NCI, NCM).
...
-spec map_user_selector(gleam@erlang@process:selector(message(NCP))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(NCP)).
...
-spec convert_loop(
    fun((NCU, message(NCV), connection(NCV)) -> next(NCU, message(NCV)))
) -> fun((NCU, glisten@internal@handler:loop_message(NCV), glisten@internal@handler:connection(NCV)) -> glisten@internal@handler:next(NCU, glisten@internal@handler:loop_message(NCV))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(NDA)) -> {NDC,
        gleam@option:option(gleam@erlang@process:selector(NDA))})
) -> fun((glisten@internal@handler:connection(NDA)) -> {NDC,
    gleam@option:option(gleam@erlang@process:selector(NDA))}).
...
-spec new(
    fun((connection(NDI)) -> {NDK,
        gleam@option:option(gleam@erlang@process:selector(NDI))}),
    fun((NDK, message(NDI), connection(NDI)) -> next(NDK, message(NDI)))
) -> builder(NDK, NDI).
new(On_init, Loop) ->
...
-spec with_close(builder(NDR, NDS), fun((NDR) -> nil)) -> builder(NDR, NDS).
...
-spec with_pool_size(builder(NDX, NDY), integer()) -> builder(NDX, NDY).
...
?DOC(false).
-spec with_http2(builder(NED, NEE)) -> builder(NED, NEE).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_ipv6(builder(NEP, NEQ)) -> builder(NEP, NEQ).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(NEV, NEW), binary(), binary()) -> builder(NEV, NEW).
...
-spec bind(builder(NEJ, NEK), binary()) -> builder(NEJ, NEK).
