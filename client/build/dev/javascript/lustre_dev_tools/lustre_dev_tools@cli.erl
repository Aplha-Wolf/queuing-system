...
-spec string(
    ...
    fun((fun((glint:flags()) -> {ok, binary()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AGOP))
) -> glint:command(AGOP).
...
-spec flag(
    ...
    list(binary()),
    lustre_dev_tools@project:project(),
    fun((binary()) -> glint:flag(AGPG)),
    fun((gleam@dict:dict(binary(), tom:toml()), list(binary())) -> {ok, AGPG} |
        {error, tom:get_error()}),
    binary(),
    fun((fun((glint:flags()) -> {ok, AGPG} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AGPP))
) -> glint:command(AGPP).
...
-spec bool(
    ...
    fun((fun((glint:flags()) -> {ok, boolean()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AGOJ))
) -> glint:command(AGOJ).
...
-spec string_list(
    ...
    fun((fun((glint:flags()) -> {ok, list(binary())} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AGOW))
) -> glint:command(AGOW).
...
-spec int(
    ...
    fun((fun((glint:flags()) -> {ok, integer()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AGPC))
) -> glint:command(AGPC).
