    ...
    CreateQueue -> {
      ...
      case pid, qid {
        Ok(priority_id), Ok(quetype_id) if priority_id > 0 && quetype_id > 0 -> {
          case frontdesk_mod.create_queue(model.frontdesk_service, priority_id, quetype_id) {
            frontdesk_mod.QueueCreated ->
              #(Model(..model, message: "Queue created", message_ok: True, selected_priority: "0", selected_quetype: "0"), none())
            frontdesk_mod.QueueFailed(err) ->
              #(Model(..model, message: err, message_ok: False), none())
          }
        }
        _, _ -> #(Model(..model, message: "Select priority and queue type", message_ok: False), none())
      }
    }
    NoOp -> #(model, none())
  }
...
pub fn view(model: Model) -> Element(Msg) {
  ...
  html.div(
    [attribute.style("background-color", colors.background), attribute.style("color", colors.text_primary), attribute.style("height", "100vh")],
    [
...
fn header_view(title: String, bg: String, fg: String) -> Element(Msg) {
  html.div(
    ...
    [kit.div_with_class_and_style("p-4 text-xl font-extrabold", bg, [], [html.text(title)])],
  )
}
...
fn priority_grid(
  ...
  let buttons = list.map(priorities, fn(p) {
  ...
  html.div(
    [attribute.style("display", "flex"), attribute.style("flex-wrap", "wrap"), attribute.style("gap", "4px"), attribute.style("padding", "16px")],
    buttons,
  )
}
...
fn quetype_grid(
  ...
  let buttons = list.map(quetypes, fn(q) {
...
fn create_button(
  ...
  let has_selection = model.selected_priority != "0" && model.selected_quetype != "0"
