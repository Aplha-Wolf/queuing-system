...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(EYG)) -> {ok, EYG} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(EYA)) -> {ok, EYA} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(EXW)) -> {ok, EXW} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(EYM), fun((EYM) -> json())) -> json().
...
-spec array(list(EYQ), fun((EYQ) -> json())) -> json().
