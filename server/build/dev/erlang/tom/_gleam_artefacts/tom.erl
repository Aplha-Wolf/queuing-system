...
-spec push_key({ok, AEBN} | {error, get_error()}, binary()) -> {ok, AEBN} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {AEDS, list(binary())}} |
        {error, parse_error()})
) -> {ok, {AEDS, list(binary())}} | {error, parse_error()}.
...
-spec do(
    {ok, {AEED, list(binary())}} | {error, parse_error()},
    fun((AEED, list(binary())) -> {ok, AEEG} | {error, parse_error()})
) -> {ok, AEEG} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {AEEL, list(binary())}} |
        {error, parse_error()})
) -> {ok, {AEEL, list(binary())}} | {error, parse_error()}.
