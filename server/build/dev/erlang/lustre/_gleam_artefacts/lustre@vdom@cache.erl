...
-opaque cache(PYE) :: {cache,
        events(PYE),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(PYE)), lustre@vdom@vnode:element(PYE)),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(PYE)), lustre@vdom@vnode:element(PYE)),
        list(binary()),
        list(binary())}.

-opaque events(PYF) :: {events,
        lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(PYF))),
        lustre@internals@mutable_map:mutable_map(binary(), child(PYF))}.

-type child(PYG) :: {child,
        fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
        events(PYG)}.

-type added_children(PYH) :: {added_children,
        lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(PYH))),
        lustre@internals@mutable_map:mutable_map(binary(), child(PYH)),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(PYH)), lustre@vdom@vnode:element(PYH))}.

-opaque decoded_event(PYI) :: {decoded_event,
        binary(),
        lustre@vdom@vattr:handler(PYI)} |
...
?DOC(false).
-spec events(cache(PYT)) -> events(PYT).
events(Cache) ->
    erlang:element(2, Cache).

-file("src/lustre/vdom/cache.gleam", 106).
?DOC(false).
-spec update_events(cache(PYW), events(PYW)) -> cache(PYW).
...
?DOC(false).
-spec memos(cache(PZA)) -> lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(PZA)), lustre@vdom@vnode:element(PZA)).
memos(Cache) ->
    erlang:element(3, Cache).
...
-spec get_old_memo(
    cache(PZD),
    fun(() -> lustre@vdom@vnode:element(PZD)),
    fun(() -> lustre@vdom@vnode:element(PZD))
...
-spec keep_memo(
    cache(PZI),
    fun(() -> lustre@vdom@vnode:element(PZI)),
    fun(() -> lustre@vdom@vnode:element(PZI))
) -> cache(PZI).
...
-spec add_memo(
    cache(PZN),
    fun(() -> lustre@vdom@vnode:element(PZN)),
...
?DOC(false).
-spec do_add_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QAE))),
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QAE))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QAE))).
...
-spec add_event(
    ...
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(PZZ))
) -> events(PZZ).
...
-spec do_remove_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QAS))),
    lustre@vdom@path:path(),
    binary()
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QAS))).
...
?DOC(false).
-spec remove_event(events(QAP), lustre@vdom@path:path(), binary()) -> events(QAP).
...
?DOC(false).
-spec add_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QBH))),
    lustre@vdom@path:path(),
    list(lustre@vdom@vattr:attribute(QBH))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QBH))).
...
-spec do_add_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QBZ))),
    lustre@internals@mutable_map:mutable_map(binary(), child(QBZ)),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(QBZ)), lustre@vdom@vnode:element(QBZ)),
    lustre@vdom@path:path(),
    integer(),
...
-spec add_child(
    ...
    lustre@vdom@path:path(),
    integer(),
    lustre@vdom@vnode:element(QBB)
) -> {cache(QBB), events(QBB)}.
...
?DOC(false).
-spec from_node(lustre@vdom@vnode:element(PYN)) -> cache(PYN).
from_node(Root) ->
    Cache = new(),
...
-spec remove_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QCQ))),
    lustre@vdom@path:path(),
...
-spec do_remove_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QDB))),
    lustre@internals@mutable_map:mutable_map(binary(), child(QDB)),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(QDB)), lustre@vdom@vnode:element(QDB)),
    lustre@vdom@path:path(),
    integer(),
...
-spec replace_child(
    ...
    lustre@vdom@path:path(),
    integer(),
    lustre@vdom@vnode:element(QDN),
    lustre@vdom@vnode:element(QDN)
) -> {cache(QDN), events(QDN)}.
...
?DOC(false).
-spec dispatch(cache(QEB), decoded_event(QEB)) -> {cache(QEB),
    {ok, lustre@vdom@vattr:handler(QEB)} | {error, nil}}.
...
?DOC(false).
-spec handle(cache(QEF), binary(), binary(), gleam@dynamic:dynamic_()) -> {cache(QEF),
    {ok, lustre@vdom@vattr:handler(QEF)} | {error, nil}}.
