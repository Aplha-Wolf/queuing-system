...
-spec on_attribute_change(binary(), fun((binary()) -> {ok, SMC} | {error, nil})) -> lustre@runtime@app:option(SMC).
...
-spec on_property_change(binary(), gleam@dynamic@decode:decoder(SMG)) -> lustre@runtime@app:option(SMG).
...
-spec on_context_change(binary(), gleam@dynamic@decode:decoder(SMJ)) -> lustre@runtime@app:option(SMJ).
...
-spec on_form_autofill(fun((binary()) -> SMO)) -> lustre@runtime@app:option(SMO).
...
-spec on_form_reset(SMQ) -> lustre@runtime@app:option(SMQ).
...
-spec on_form_restore(fun((binary()) -> SMS)) -> lustre@runtime@app:option(SMS).
...
-spec on_connect(SNA) -> lustre@runtime@app:option(SNA).
on_connect(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_adopt(SNC) -> lustre@runtime@app:option(SNC).
on_adopt(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_disconnect(SNE) -> lustre@runtime@app:option(SNE).
...
-spec prerender(
    lustre@runtime@app:app(nil, any(), SOM),
    binary(),
