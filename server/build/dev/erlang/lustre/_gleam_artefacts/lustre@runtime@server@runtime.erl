...
-type state(RAU, RAV) :: {state,
        gleam@erlang@process:subject(message(RAV)),
        gleam@erlang@process:selector(message(RAV)),
        gleam@erlang@process:selector(message(RAV)),
        RAU,
        fun((RAU, RAV) -> {RAU, lustre@effect:effect(RAV)}),
        fun((RAU) -> lustre@vdom@vnode:element(RAV)),
        config(RAV),
        lustre@vdom@vnode:element(RAV),
        lustre@vdom@cache:cache(RAV),
        gleam@dict:dict(binary(), gleam@json:json()),
        gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(RAV)), gleam@erlang@process:monitor()),
        gleam@set:set(fun((lustre@runtime@transport:client_message(RAV)) -> nil))}.

-type config(RAW) :: {config,
        boolean(),
        boolean(),
        gleam@dict:dict(binary(), fun((binary()) -> {ok, RAW} | {error, nil})),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(RAW)),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(RAW)),
        gleam@option:option(RAW),
        gleam@option:option(RAW)}.

-type message(RAX) :: {client_dispatched_message,
        lustre@runtime@transport:server_message()} |
    {client_registered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(RAX))} |
    {client_deregistered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(RAX))} |
    {client_registered_callback,
        fun((lustre@runtime@transport:client_message(RAX)) -> nil)} |
    {client_deregistered_callback,
        fun((lustre@runtime@transport:client_message(RAX)) -> nil)} |
    {effect_added_selector, gleam@erlang@process:selector(message(RAX))} |
    {effect_dispatched_message, RAX} |
...
-spec handle_attribute_change(
    gleam@dict:dict(binary(), fun((binary()) -> {ok, RCG} | {error, nil})),
    binary(),
    binary()
) -> {ok, RCG} | {error, nil}.
...
-spec handle_property_change(
    gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(RCN)),
    binary(),
...
?DOC(false).
-spec handle_effect(
    gleam@erlang@process:subject(message(RCT)),
    lustre@effect:effect(RCT)
) -> nil.
...
?DOC(false).
-spec broadcast(
    gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(RCX)), gleam@erlang@process:monitor()),
    gleam@set:set(fun((lustre@runtime@transport:client_message(RCX)) -> nil)),
    lustre@runtime@transport:client_message(RCX)
) -> nil.
...
?DOC(false).
-spec loop(state(RBQ, RBR), message(RBR)) -> gleam@otp@actor:next(state(RBQ, RBR), message(RBR)).
loop(State, Message) ->
    case Message of
...
?DOC(false).
-spec start(
    gleam@option:option(gleam@erlang@process:name(message(RBB))),
    fun((RBF) -> {RBG, lustre@effect:effect(RBB)}),
    fun((RBG, RBB) -> {RBG, lustre@effect:effect(RBB)}),
    fun((RBG) -> lustre@vdom@vnode:element(RBB)),
    config(RBB),
    RBF
) -> {ok, gleam@otp@actor:started(gleam@erlang@process:subject(message(RBB)))} |
