...
-spec fragment(list(lustre@vdom@vnode:element(OEI))) -> lustre@vdom@vnode:element(OEI).
fragment(Children) ->
...
-spec unsafe_raw_html(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(OEM)),
    binary()
) -> lustre@vdom@vnode:element(OEM).
...
-spec memo(
    ...
    fun(() -> lustre@vdom@vnode:element(OER))
) -> lustre@vdom@vnode:element(OER).
...
-spec map(lustre@vdom@vnode:element(OEV), fun((OEV) -> OEX)) -> lustre@vdom@vnode:element(OEX).
map(Element, F) ->
