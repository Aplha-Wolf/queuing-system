...
-type diff(QOY) :: {diff,
        lustre@vdom@patch:patch(QOY),
        lustre@vdom@cache:cache(QOY)}.

-type partial_diff(QOZ) :: {partial_diff,
        lustre@vdom@patch:patch(QOZ),
        lustre@vdom@cache:cache(QOZ),
        lustre@vdom@cache:events(QOZ)}.

-type attribute_change(QPA) :: {attribute_change,
        list(lustre@vdom@vattr:attribute(QPA)),
        list(lustre@vdom@vattr:attribute(QPA)),
        lustre@vdom@cache:events(QPA)}.
...
-spec diff_attributes(
    boolean(),
    lustre@vdom@path:path(),
    lustre@vdom@cache:events(QQD),
    list(lustre@vdom@vattr:attribute(QQD)),
    list(lustre@vdom@vattr:attribute(QQD)),
    list(lustre@vdom@vattr:attribute(QQD)),
    list(lustre@vdom@vattr:attribute(QQD))
) -> attribute_change(QQD).
...
?DOC(false).
-spec do_diff(
    list(lustre@vdom@vnode:element(QPG)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(QPG)),
    list(lustre@vdom@vnode:element(QPG)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(QPG)),
    ...
    integer(),
    integer(),
    list(lustre@vdom@patch:change(QPG)),
    list(lustre@vdom@patch:patch(QPG)),
    lustre@vdom@path:path(),
    lustre@vdom@cache:cache(QPG),
    lustre@vdom@cache:events(QPG)
) -> partial_diff(QPG).
do_diff(
    Old,
    Old_keyed,
...
?DOC(false).
-spec diff(
    lustre@vdom@cache:cache(QPB),
    lustre@vdom@vnode:element(QPB),
    lustre@vdom@vnode:element(QPB)
) -> diff(QPB).
diff(Cache, Old, New) ->
