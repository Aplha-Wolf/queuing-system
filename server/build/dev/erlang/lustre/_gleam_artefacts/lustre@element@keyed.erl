...
-spec do_extract_keyed_children(
    list({binary(), lustre@vdom@vnode:element(UJC)}),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(UJC)),
    list(lustre@vdom@vnode:element(UJC))
) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(UJC)),
    list(lustre@vdom@vnode:element(UJC))}.
...
-spec extract_keyed_children(list({binary(), lustre@vdom@vnode:element(UIU)})) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(UIU)),
    list(lustre@vdom@vnode:element(UIU))}.
...
-spec element(
    binary(),
    list(lustre@vdom@vattr:attribute(UHA)),
    list({binary(), lustre@vdom@vnode:element(UHA)})
) -> lustre@vdom@vnode:element(UHA).
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(UHG)),
    list({binary(), lustre@vdom@vnode:element(UHG)})
) -> lustre@vdom@vnode:element(UHG).
...
-spec fragment(list({binary(), lustre@vdom@vnode:element(UHM)})) -> lustre@vdom@vnode:element(UHM).
fragment(Children) ->
    {Keyed_children, Children@1} = extract_keyed_children(Children),
...
-spec ul(
    ...
    list({binary(), lustre@vdom@vnode:element(UHQ)})
...
-spec ol(
    list(lustre@vdom@vattr:attribute(UHW)),
    list({binary(), lustre@vdom@vnode:element(UHW)})
) -> lustre@vdom@vnode:element(UHW).
ol(Attributes, Children) ->
    element(<<"ol"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 152).
-spec 'div'(
    list(lustre@vdom@vattr:attribute(UIC)),
    list({binary(), lustre@vdom@vnode:element(UIC)})
) -> lustre@vdom@vnode:element(UIC).
'div'(Attributes, Children) ->
    element(<<"div"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 159).
-spec tbody(
    list(lustre@vdom@vattr:attribute(UII)),
    list({binary(), lustre@vdom@vnode:element(UII)})
) -> lustre@vdom@vnode:element(UII).
tbody(Attributes, Children) ->
    element(<<"tbody"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 166).
-spec dl(
    list(lustre@vdom@vattr:attribute(UIO)),
    list({binary(), lustre@vdom@vnode:element(UIO)})
) -> lustre@vdom@vnode:element(UIO).
