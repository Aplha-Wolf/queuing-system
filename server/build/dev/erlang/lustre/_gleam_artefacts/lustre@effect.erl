...
-opaque effect(NJH) :: {effect,
        list(fun((actions(NJH)) -> nil)),
        list(fun((actions(NJH)) -> nil)),
        list(fun((actions(NJH)) -> nil))}.

-type actions(NJI) :: {actions,
        fun((NJI) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(NJI)) -> nil),
...
-spec do_comap_select(
    actions(NKS),
    gleam@erlang@process:selector(NKU),
    fun((NKU) -> NKS)
) -> nil.
...
-spec do_comap_actions(actions(NKO), fun((NKQ) -> NKO)) -> actions(NKQ).
...
-spec do_map(list(fun((actions(NKI)) -> nil)), fun((NKI) -> NKL)) -> list(fun((actions(NKL)) -> nil)).
do_map(Effects, F) ->
    gleam@list:map(
        Effects,
        fun(Effect) ->
...
-spec map(effect(NKE), fun((NKE) -> NKG)) -> effect(NKG).
map(Effect, F) ->
    {effect,
...
-spec perform(
    ...
    fun((NKW) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(NKW)) -> nil),
    fun(() -> gleam@dynamic:dynamic_()),
    fun((binary(), gleam@json:json()) -> nil)
) -> nil.
...
-spec from(fun((fun((NJL) -> nil)) -> nil)) -> effect(NJL).
from(Effect) ->
    Task = fun(Actions) ->
...
-spec before_paint(fun((fun((NJN) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(NJN).
before_paint(Effect) ->
    Task = fun(Actions) ->
...
-spec after_paint(fun((fun((NJP) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(NJP).
after_paint(Effect) ->
    Task = fun(Actions) ->
        Root = (erlang:element(5, Actions))(),
        Dispatch = erlang:element(2, Actions),
...
?DOC(false).
-spec select(
    fun((fun((NJT) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(NJT))
) -> effect(NJT).
select(Sel) ->
    Task = fun(Actions) ->
...
-spec batch(list(effect(NKA))) -> effect(NKA).
batch(Effects) ->
    gleam@list:fold(
        Effects,
        {effect, [], [], []},
