...
-type runtime(SZZ) :: any() | {gleam_phantom, SZZ}.
...
-spec application(
    fun((TAV) -> {TAW, lustre@effect:effect(TAX)}),
    fun((TAW, TAX) -> {TAW, lustre@effect:effect(TAX)}),
    fun((TAW) -> lustre@vdom@vnode:element(TAX))
) -> lustre@runtime@app:app(TAV, TAW, TAX).
...
-spec element(lustre@vdom@vnode:element(TAI)) -> lustre@runtime@app:app(any(), nil, TAI).
element(View) ->
    application(
...
-spec simple(
    fun((TAO) -> TAP),
    fun((TAP, TAQ) -> TAP),
    fun((TAP) -> lustre@vdom@vnode:element(TAQ))
) -> lustre@runtime@app:app(TAO, TAP, TAQ).
...
-spec component(
    fun((TBE) -> {TBF, lustre@effect:effect(TBG)}),
    fun((TBF, TBG) -> {TBF, lustre@effect:effect(TBG)}),
    fun((TBF) -> lustre@vdom@vnode:element(TBG)),
    list(lustre@runtime@app:option(TBG))
) -> lustre@runtime@app:app(TBE, TBF, TBG).
...
-spec named(
    lustre@runtime@app:app(TBP, TBQ, TBR),
    gleam@erlang@process:name(lustre@runtime@server@runtime:message(TBR))
) -> lustre@runtime@app:app(TBP, TBQ, TBR).
named(App, Name) ->
    {app,
        {some, Name},
...
-spec do_start(lustre@runtime@app:app(TCJ, any(), TCL), binary(), TCJ) -> {ok,
        runtime(TCL)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.
...
-spec supervised(lustre@runtime@app:app(TDB, any(), TDD), TDB) -> gleam@otp@supervision:child_specification(gleam@erlang@process:subject(lustre@runtime@server@runtime:message(TDD))).
...
-spec factory(lustre@runtime@app:app(TDK, any(), TDM)) -> gleam@otp@factory_supervisor:builder(TDK, gleam@erlang@process:subject(lustre@runtime@server@runtime:message(TDM))).
factory(App) ->
...
-spec send(runtime(TEB), lustre@runtime@server@runtime:message(TEB)) -> nil.
...
-spec dispatch(TEE) -> lustre@runtime@server@runtime:message(TEE).
dispatch(Message) ->
...
-spec start(lustre@runtime@app:app(TCA, any(), TCC), binary(), TCA) -> {ok,
        runtime(TCC)} |
    {error, error()}.
...
-spec start_server_component(lustre@runtime@app:app(TCS, any(), TCU), TCS) -> {ok,
        runtime(TCU)} |
    {error, error()}.
