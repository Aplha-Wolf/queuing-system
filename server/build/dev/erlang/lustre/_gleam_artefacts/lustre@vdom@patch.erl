...
-type patch(PND) :: {patch,
        integer(),
        integer(),
        list(change(PND)),
        list(patch(PND))}.

-type change(PNE) :: {replace_text, integer(), binary()} |
    ...
    {replace, integer(), integer(), lustre@vdom@vnode:element(PNE)} |
    {remove, integer(), integer()} |
    {insert, integer(), list(lustre@vdom@vnode:element(PNE)), integer()}.

-file("src/lustre/vdom/patch.gleam", 66).
?DOC(false).
-spec new(integer(), integer(), list(change(PNF)), list(patch(PNF))) -> patch(PNF).
...
?DOC(false).
-spec add_child(patch(POI), patch(POI)) -> patch(POI).
...
-spec replace_to_json(
    ...
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(PQZ)), lustre@vdom@vnode:element(PQZ))
) -> gleam@json:json().
...
-spec insert_to_json(
    ...
    integer(),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(PRD)), lustre@vdom@vnode:element(PRD))
) -> gleam@json:json().
...
-spec change_to_json(
    change(POP),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(POP)), lustre@vdom@vnode:element(POP))
) -> gleam@json:json().
...
-spec to_json(
    patch(POM),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(POM)), lustre@vdom@vnode:element(POM))
) -> gleam@json:json().
to_json(Patch, Memos) ->
...
?DOC(false).
-spec replace(integer(), lustre@vdom@vnode:element(PNZ)) -> change(PNZ).
replace(Index, With) ->
    {replace, 5, Index, With}.

-file("src/lustre/vdom/patch.gleam", 116).
?DOC(false).
-spec insert(list(lustre@vdom@vnode:element(POC)), integer()) -> change(POC).
