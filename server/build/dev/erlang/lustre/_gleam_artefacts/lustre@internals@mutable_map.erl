...
-type mutable_map(NPQ, NPR) :: any() | {gleam_phantom, NPQ, NPR}.
...
?DOC(false).
-spec unsafe_get(mutable_map(NPW, NPX), NPW) -> NPX.
unsafe_get(Map, Key) ->
    maps:get(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 34).
?DOC(false).
-spec get_or_compute(mutable_map(NQE, NQF), NQE, fun(() -> NQF)) -> NQF.
...
?DOC(false).
-spec has_key(mutable_map(NQO, any()), NQO) -> boolean().
has_key(Map, Key) ->
    maps:is_key(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 61).
?DOC(false).
-spec insert(mutable_map(NQW, NQX), NQW, NQX) -> mutable_map(NQW, NQX).
insert(Map, Key, Value) ->
    maps:put(Key, Value, Map).

-file("src/lustre/internals/mutable_map.gleam", 79).
?DOC(false).
-spec delete(mutable_map(NRI, NRJ), NRI) -> mutable_map(NRI, NRJ).
delete(Map, Key) ->
    maps:remove(Key, Map).
