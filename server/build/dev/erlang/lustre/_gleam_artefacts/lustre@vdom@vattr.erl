...
-type attribute(MKF) :: {attribute, integer(), binary(), binary()} |
    {property, integer(), binary(), gleam@json:json()} |
    {event,
        integer(),
        binary(),
        gleam@dynamic@decode:decoder(handler(MKF)),
        list(binary()),
        event_behaviour(),
        event_behaviour(),
        integer(),
...
-type handler(MKG) :: {handler, boolean(), boolean(), MKG}.
...
?DOC(false).
-spec merge(list(attribute(MKV)), list(attribute(MKV))) -> list(attribute(MKV)).
...
?DOC(false).
-spec compare(attribute(MLC), attribute(MLC)) -> gleam@order:order().
compare(A, B) ->
    gleam@string:compare(erlang:element(3, A), erlang:element(3, B)).

-file("src/lustre/vdom/vattr.gleam", 91).
?DOC(false).
-spec prepare(list(attribute(MKQ))) -> list(attribute(MKQ)).
prepare(Attributes) ->
    case Attributes of
        [] ->
            Attributes;
...
-spec event(
    binary(),
    gleam@dynamic@decode:decoder(handler(MKL)),
    list(binary()),
    event_behaviour(),
    event_behaviour(),
    integer(),
