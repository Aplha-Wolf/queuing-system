...
-opaque app(TYB, TYC, TYD) :: {app,
        fun((TYB) -> {TYC, lustre@effect:effect(TYD)}),
        fun((TYC, TYD) -> {TYC, lustre@effect:effect(TYD)}),
        fun((TYC) -> lustre@vdom@vnode:element(TYD))}.

-opaque simulation(TYE, TYF) :: {simulation,
        fun((TYE, TYF) -> {TYE, lustre@effect:effect(TYF)}),
        fun((TYE) -> lustre@vdom@vnode:element(TYF)),
        list(event(TYF)),
        TYE,
        lustre@vdom@vnode:element(TYF)}.

-type event(TYG) :: {dispatch, TYG} |
...
-spec simple(
    fun((TYH) -> TYI),
    fun((TYI, TYJ) -> TYI),
    fun((TYI) -> lustre@vdom@vnode:element(TYJ))
) -> app(TYH, TYI, TYJ).
...
-spec application(
    fun((TYO) -> {TYP, lustre@effect:effect(TYQ)}),
    fun((TYP, TYQ) -> {TYP, lustre@effect:effect(TYQ)}),
    fun((TYP) -> lustre@vdom@vnode:element(TYQ))
) -> app(TYO, TYP, TYQ).
...
-spec start(app(TYX, TYY, TYZ), TYX) -> simulation(TYY, TYZ).
start(App, Args) ->
...
-spec message(simulation(TZF, TZG), TZG) -> simulation(TZF, TZG).
...
-spec problem(simulation(UAL, UAM), binary(), binary()) -> simulation(UAL, UAM).
...
-spec model(simulation(UAR, any())) -> UAR.
model(Simulation) ->
    erlang:element(5, Simulation).
...
-spec view(simulation(any(), UAW)) -> lustre@vdom@vnode:element(UAW).
view(Simulation) ->
    erlang:element(6, Simulation).
...
-spec history(simulation(any(), UBB)) -> list(event(UBB)).
history(Simulation) ->
...
-spec click(simulation(TZS, TZT), lustre@dev@query:'query'()) -> simulation(TZS, TZT).
...
-spec input(simulation(TZY, TZZ), lustre@dev@query:'query'(), binary()) -> simulation(TZY, TZZ).
