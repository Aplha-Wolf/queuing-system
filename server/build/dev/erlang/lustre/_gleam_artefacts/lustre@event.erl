...
-spec on(binary(), gleam@dynamic@decode:decoder(VIB)) -> lustre@vdom@vattr:attribute(VIB).
on(Name, Handler) ->
...
-spec advanced(
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(VIE))
) -> lustre@vdom@vattr:attribute(VIE).
...
-spec handler(VII, boolean(), boolean()) -> lustre@vdom@vattr:handler(VII).
...
-spec prevent_default(lustre@vdom@vattr:attribute(VIK)) -> lustre@vdom@vattr:attribute(VIK).
...
-spec stop_propagation(lustre@vdom@vattr:attribute(VIN)) -> lustre@vdom@vattr:attribute(VIN).
...
-spec debounce(lustre@vdom@vattr:attribute(VIQ), integer()) -> lustre@vdom@vattr:attribute(VIQ).
...
-spec throttle(lustre@vdom@vattr:attribute(VIT), integer()) -> lustre@vdom@vattr:attribute(VIT).
...
-spec on_keypress(fun((binary()) -> VJK)) -> lustre@vdom@vattr:attribute(VJK).
on_keypress(Msg) ->
    on(
        <<"keypress"/utf8>>,
        begin
...
-spec on_keydown(fun((binary()) -> VJM)) -> lustre@vdom@vattr:attribute(VJM).
on_keydown(Msg) ->
    on(
        <<"keydown"/utf8>>,
        begin
...
-spec on_keyup(fun((binary()) -> VJO)) -> lustre@vdom@vattr:attribute(VJO).
on_keyup(Msg) ->
    on(
        <<"keyup"/utf8>>,
        begin
...
-spec on_input(fun((binary()) -> VJQ)) -> lustre@vdom@vattr:attribute(VJQ).
on_input(Msg) ->
    on(
        <<"input"/utf8>>,
        begin
...
-spec on_change(fun((binary()) -> VJS)) -> lustre@vdom@vattr:attribute(VJS).
on_change(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_check(fun((boolean()) -> VJU)) -> lustre@vdom@vattr:attribute(VJU).
on_check(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_submit(fun((list({binary(), binary()})) -> VJX)) -> lustre@vdom@vattr:attribute(VJX).
on_submit(Msg) ->
    _pipe@2 = on(
        <<"submit"/utf8>>,
        begin
...
-spec on_focus(VKB) -> lustre@vdom@vattr:attribute(VKB).
on_focus(Msg) ->
    on(<<"focus"/utf8>>, gleam@dynamic@decode:success(Msg)).

-file("src/lustre/event.gleam", 352).
-spec on_blur(VKD) -> lustre@vdom@vattr:attribute(VKD).
on_blur(Msg) ->
