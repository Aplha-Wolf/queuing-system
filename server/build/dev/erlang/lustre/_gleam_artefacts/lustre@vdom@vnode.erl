...
-type element(NSY) :: {fragment,
        integer(),
        binary(),
        list(element(NSY)),
        lustre@internals@mutable_map:mutable_map(binary(), element(NSY))} |
    {element,
        integer(),
        binary(),
        binary(),
        binary(),
        list(lustre@vdom@vattr:attribute(NSY)),
        list(element(NSY)),
        lustre@internals@mutable_map:mutable_map(binary(), element(NSY)),
        boolean(),
        boolean()} |
...
?DOC(false).
-spec to_keyed(binary(), element(NUM)) -> element(NUM).
to_keyed(Key, Node) ->
    case Node of
...
-spec fragment(
    binary(),
    list(element(NTG)),
    lustre@internals@mutable_map:mutable_map(binary(), element(NTG))
) -> element(NTG).
...
-spec element(
    ...
    list(element(NTN)),
    lustre@internals@mutable_map:mutable_map(binary(), element(NTN)),
    boolean(),
    boolean()
) -> element(NTN).
element(
...
?DOC(false).
-spec map(element(NUC), fun((NUC) -> NUE)) -> element(NUE).
map(Element, Mapper) ->
    case Element of
...
?DOC(false).
-spec memo(binary(), list(lustre@internals@ref:ref()), fun(() -> element(NUH))) -> element(NUH).
...
-spec fragment_to_json(
    ...
    binary(),
    list(element(NUP)),
    lustre@internals@mutable_map:mutable_map(fun(() -> element(NUP)), element(NUP))
) -> gleam@json:json().
...
-spec element_to_json(
    ...
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(NUP)),
    list(element(NUP)),
    lustre@internals@mutable_map:mutable_map(fun(() -> element(NUP)), element(NUP))
) -> gleam@json:json().
