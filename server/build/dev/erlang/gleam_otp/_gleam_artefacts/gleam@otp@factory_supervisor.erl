...
-opaque supervisor(HZC, HZD) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(HZC, HZD))}.

-type message(HZE, HZF) :: any() | {gleam_phantom, HZE, HZF}.

-opaque builder(HZG, HZH) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((HZG) -> {ok, gleam@otp@actor:started(HZH)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(HZG, HZH)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(HZI, HZJ) :: {local,
        gleam@erlang@process:name(message(HZI, HZJ))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(HZK) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, HZK}.
...
-type erlang_child_spec_property(HZL, HZM) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((HZL) -> {ok, gleam@otp@actor:started(HZM)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(HZN, HZO))) -> supervisor(HZN, HZO).
get_by_name(Name) ->
...
-spec named(builder(IAE, IAF), gleam@erlang@process:name(message(IAE, IAF))) -> builder(IAE, IAF).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(IAN, IAO), integer(), integer()) -> builder(IAN, IAO).
...
-spec timeout(builder(IAT, IAU), integer()) -> builder(IAT, IAU).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(IAZ, IBA), gleam@otp@supervision:restart()) -> builder(IAZ, IBA).
...
-spec start(builder(IBF, IBG)) -> {ok,
        gleam@otp@actor:started(supervisor(IBF, IBG))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(ICE, ICF)) -> gleam@otp@supervision:child_specification(supervisor(ICE, ICF)).
supervised(Builder) ->
...
-spec start_child(supervisor(ICL, ICM), ICL) -> {ok,
        gleam@otp@actor:started(ICM)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((IDJ) -> {ok, gleam@otp@actor:started(IDK)} |
        {error, gleam@otp@actor:start_error()}),
    IDJ
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), IDK, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((HZU) -> {ok, gleam@otp@actor:started(HZV)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(HZU, HZV).
...
-spec supervisor_child(
    fun((HZZ) -> {ok, gleam@otp@actor:started(IAA)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(HZZ, IAA).
