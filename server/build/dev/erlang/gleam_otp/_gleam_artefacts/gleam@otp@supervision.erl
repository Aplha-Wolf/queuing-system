...
-type child_specification(HWO) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(HWO)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(HWP)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(HWP).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(HWU)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(HWU).
supervisor(Start) ->
...
-spec significant(child_specification(HWZ), boolean()) -> child_specification(HWZ).
...
-spec timeout(child_specification(HXC), integer()) -> child_specification(HXC).
timeout(Child, Ms) ->
...
-spec restart(child_specification(HXF), restart()) -> child_specification(HXF).
...
-spec map_data(child_specification(HXI), fun((HXI) -> HXK)) -> child_specification(HXK).
