...
-type message(HDJ) :: {message, HDJ} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(HDK, HDL) :: {continue,
        HDK,
        gleam@option:option(gleam@erlang@process:selector(HDL))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(HDM, HDN) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        HDM,
        gleam@erlang@process:selector(message(HDN)),
        gleam@otp@system:debug_state(),
        fun((HDM, HDN) -> next(HDM, HDN))}.

-type started(HDO) :: {started, gleam@erlang@process:pid_(), HDO}.

-opaque initialised(HDP, HDQ, HDR) :: {initialised,
        HDP,
        gleam@option:option(gleam@erlang@process:selector(HDQ)),
        HDR}.

-opaque builder(HDS, HDT, HDU) :: {builder,
        fun((gleam@erlang@process:subject(HDT)) -> {ok,
                initialised(HDS, HDT, HDU)} |
            {error, binary()}),
        integer(),
        fun((HDS, HDT) -> next(HDS, HDT)),
        gleam@option:option(gleam@erlang@process:name(HDT))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(HDV) :: {ack, {ok, HDV} | {error, binary()}} |
...
-spec continue(HEA) -> next(HEA, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(HEM, HEN), gleam@erlang@process:selector(HEN)) -> next(HEM, HEN).
...
-spec initialised(HET) -> initialised(HET, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(HEY, any(), HFA),
    gleam@erlang@process:selector(HFE)
) -> initialised(HEY, HFE, HFA).
...
-spec returning(initialised(HFJ, HFK, any()), HFP) -> initialised(HFJ, HFK, HFP).
...
-spec new(HFT) -> builder(HFT, HFU, gleam@erlang@process:subject(HFU)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(HFZ)) -> {ok, initialised(HGB, HFZ, HGC)} |
        {error, binary()})
) -> builder(HGB, HFZ, HGC).
...
-spec on_message(builder(HGL, HGM, HGN), fun((HGL, HGM) -> next(HGL, HGM))) -> builder(HGL, HGM, HGN).
...
-spec named(builder(HGW, HGX, HGY), gleam@erlang@process:name(HGX)) -> builder(HGW, HGX, HGY).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(HHL))) -> gleam@erlang@process:selector(message(HHL)).
...
-spec receive_message(self(any(), HHH)) -> message(HHH).
...
-spec initialise_actor(
    builder(any(), any(), HIE),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, HIE} | {error, binary()})
...
-spec start(builder(any(), any(), HIR)) -> {ok, started(HIR)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(HIY), HIY) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(HJA),
    integer(),
    fun((gleam@erlang@process:subject(HJC)) -> HJA)
) -> HJC.
