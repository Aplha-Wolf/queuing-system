...
-spec map_response_body(
    fun((HAH) -> gleam@http@response:response(HAI)),
    fun((HAI) -> HAK)
) -> fun((HAH) -> gleam@http@response:response(HAK)).
...
-spec prepend_response_header(
    fun((HAM) -> gleam@http@response:response(HAN)),
    binary(),
    binary()
) -> fun((HAM) -> gleam@http@response:response(HAN)).
...
-spec ensure_post(gleam@http@request:request(HAQ)) -> {ok,
        gleam@http@request:request(HAQ)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(HAZ)) -> HBB)) -> fun((gleam@http@request:request(HAZ)) -> HBB).
