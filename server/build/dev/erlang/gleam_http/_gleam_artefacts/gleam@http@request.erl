...
-spec set_header(request(GMR), binary(), binary()) -> request(GMR).
...
-spec prepend_header(request(GMU), binary(), binary()) -> request(GMU).
...
-spec set_body(request(any()), GMZ) -> request(GMZ).
set_body(Req, Body) ->
    {request,
...
-spec map(request(GNB), fun((GNB) -> GND)) -> request(GND).
...
-spec set_query(request(GNN), list({binary(), binary()})) -> request(GNN).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(GNR), gleam@http:method()) -> request(GNR).
...
-spec set_scheme(request(GNY), gleam@http:scheme()) -> request(GNY).
...
-spec set_host(request(GOB), binary()) -> request(GOB).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(GOE), integer()) -> request(GOE).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(GOH), binary()) -> request(GOH).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(GOK), binary(), binary()) -> request(GOK).
...
-spec remove_cookie(request(GOQ), binary()) -> request(GOQ).
