...
-type response(GWL) :: {response, integer(), list({binary(), binary()}), GWL}.
...
-spec set_header(response(GXA), binary(), binary()) -> response(GXA).
...
-spec prepend_header(response(GXD), binary(), binary()) -> response(GXD).
...
-spec set_body(response(any()), GXI) -> response(GXI).
...
-spec try_map(response(GWM), fun((GWM) -> {ok, GWO} | {error, GWP})) -> {ok,
        response(GWO)} |
    {error, GWP}.
...
-spec map(response(GXK), fun((GXK) -> GXM)) -> response(GXM).
...
-spec expire_cookie(response(GXV), binary(), gleam@http@cookie:attributes()) -> response(GXV).
