...
-spec map_first({CMW, CMX}, fun((CMW) -> CMY)) -> {CMY, CMX}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({CMZ, CNA}, fun((CNA) -> CNB)) -> {CMZ, CNB}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
