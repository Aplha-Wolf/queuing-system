...
-spec map({ok, CNN} | {error, CNO}, fun((CNN) -> CNR)) -> {ok, CNR} |
    {error, CNO}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CNU} | {error, CNV}, fun((CNV) -> CNY)) -> {ok, CNU} |
    {error, CNY}.
...
-spec flatten({ok, {ok, COB} | {error, COC}} | {error, COC}) -> {ok, COB} |
    {error, COC}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, COJ} | {error, COK}, fun((COJ) -> {ok, CON} | {error, COK})) -> {ok,
        CON} |
    {error, COK}.
'try'(Result, Fun) ->
    case Result of
...
-spec unwrap({ok, COS} | {error, any()}, COS) -> COS.
...
-spec lazy_unwrap({ok, COW} | {error, any()}, fun(() -> COW)) -> COW.
...
-spec unwrap_error({ok, any()} | {error, CPB}, CPB) -> CPB.
...
-spec 'or'({ok, CPE} | {error, CPF}, {ok, CPE} | {error, CPF}) -> {ok, CPE} |
    {error, CPF}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, CPM} | {error, CPN}, fun(() -> {ok, CPM} | {error, CPN})) -> {ok,
        CPM} |
    {error, CPN}.
...
-spec all(list({ok, CPU} | {error, CPV})) -> {ok, list(CPU)} | {error, CPV}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 307).
-spec partition_loop(list({ok, CQJ} | {error, CQK}), list(CQJ), list(CQK)) -> {list(CQJ),
    list(CQK)}.
...
-spec partition(list({ok, CQC} | {error, CQD})) -> {list(CQC), list(CQD)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CQS}, CQV) -> {ok, CQV} | {error, CQS}.
...
-spec replace_error({ok, CQY} | {error, any()}, CRC) -> {ok, CQY} | {error, CRC}.
...
-spec values(list({ok, CRF} | {error, any()})) -> list(CRF).
values(Results) ->
