...
-opaque set(CWC) :: {set, gleam@dict:dict(CWC, list(nil))}.
...
-spec contains(set(CWN), CWN) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CWP), CWP) -> set(CWP).
delete(Set, Member) ->
...
-spec fold(set(CWY), CXA, fun((CXA, CWY) -> CXA)) -> CXA.
...
-spec filter(set(CXB), fun((CXB) -> boolean())) -> set(CXB).
...
-spec drop(set(CXI), list(CXI)) -> set(CXI).
drop(Set, Disallowed) ->
...
-spec take(set(CXM), list(CXM)) -> set(CXM).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 290).
-spec order(set(CXU), set(CXU)) -> {set(CXU), set(CXU)}.
order(First, Second) ->
...
-spec intersection(set(CXZ), set(CXZ)) -> set(CXZ).
...
-spec difference(set(CYD), set(CYD)) -> set(CYD).
...
-spec is_subset(set(CYH), set(CYH)) -> boolean().
...
-spec is_disjoint(set(CYK), set(CYK)) -> boolean().
...
-spec each(set(CYR), fun((CYR) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(CWK), CWK) -> set(CWK).
insert(Set, Member) ->
...
-spec from_list(list(CWV)) -> set(CWV).
from_list(Members) ->
...
-spec map(set(CXE), fun((CXE) -> CXG)) -> set(CXG).
map(Set, Fun) ->
...
-spec union(set(CXQ), set(CXQ)) -> set(CXQ).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(CYN), set(CYN)) -> set(CYN).
