...
-spec be_ok({ok, JEI} | {error, any()}) -> JEI.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, JEN}) -> JEN.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(JEQ)) -> JEQ.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
