...
-opaque subject(FQP) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(FQP)}.

-type name(FQQ) :: any() | {gleam_phantom, FQQ}.

-type do_not_leak() :: any().

-type selector(FQR) :: any() | {gleam_phantom, FQR}.
...
-spec 'receive'(subject(FRP), integer()) -> {ok, FRP} | {error, nil}.
...
-spec receive_forever(subject(FRX)) -> FRX.
...
-spec selector_receive(selector(FSB), integer()) -> {ok, FSB} | {error, nil}.
...
-spec selector_receive_forever(selector(FSF)) -> FSF.
...
-spec map_selector(selector(FSH), fun((FSH) -> FSJ)) -> selector(FSJ).
map_selector(A, B) ->
...
-spec merge_selector(selector(FSL), selector(FSL)) -> selector(FSL).
merge_selector(A, B) ->
...
-spec select_map(selector(FSW), subject(FSY), fun((FSY) -> FSW)) -> selector(FSW).
...
-spec select(selector(FSS), subject(FSS)) -> selector(FSS).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> FTG)
) -> selector(FTG).
...
-spec select_other(selector(FTK), fun((gleam@dynamic:dynamic_()) -> FTK)) -> selector(FTK).
...
-spec deselect(selector(FTB), subject(any())) -> selector(FTB).
...
-spec select_specific_monitor(selector(FTW), monitor(), fun((down()) -> FTW)) -> selector(FTW).
...
-spec select_monitors(selector(FTZ), fun((down()) -> FTZ)) -> selector(FTZ).
...
-spec select_trapped_exits(selector(FSP), fun((exit_message()) -> FSP)) -> selector(FSP).
...
-spec deselect_specific_monitor(selector(FUC), monitor()) -> selector(FUC).
...
-spec send_after(subject(FUY), integer(), FUY) -> timer().
...
-spec perform_call(
    subject(FUF),
    fun((subject(FUH)) -> FUF),
    fun((selector(FUH)) -> {ok, FUH} | {error, nil})
) -> FUH.
...
-spec call(subject(FUM), integer(), fun((subject(FUO)) -> FUM)) -> FUO.
...
-spec call_forever(subject(FUQ), fun((subject(FUS)) -> FUQ)) -> FUS.
