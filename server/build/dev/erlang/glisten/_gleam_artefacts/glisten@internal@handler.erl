...
-type message(JYH) :: {internal, internal_message()} | {user, JYH}.

-type loop_message(JYI) :: {packet, bitstring()} | {custom, JYI}.

-type loop_state(JYJ, JYK) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(JYK)),
        glisten@transport:transport(),
        JYJ}.

-type connection(JYL) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(JYL))}.

-type next(JYM, JYN) :: {continue,
        JYM,
        gleam@option:option(gleam@erlang@process:selector(JYN))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(JYO, JYP) :: {handler,
        glisten@socket:socket(),
        fun((JYO, loop_message(JYP), connection(JYP)) -> next(JYO, loop_message(JYP))),
        fun((connection(JYP)) -> {JYO,
            gleam@option:option(gleam@erlang@process:selector(JYP))}),
        gleam@option:option(fun((JYO) -> nil)),
        glisten@transport:transport(),
        glisten@socket@options:active_state()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(JZC) -> next(JZC, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(JZG, JZH), gleam@erlang@process:selector(JZH)) -> next(JZG, JZH).
...
?DOC(false).
-spec start(handler(any(), JZW)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(JZW)))} |
