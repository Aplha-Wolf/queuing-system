...
-opaque next(AAOV, AAOW) :: {continue,
        AAOV,
        gleam@option:option(gleam@erlang@process:selector(AAOW))} |
    normal_stop |
...
-opaque builder(AAOX, AAOY) :: {builder,
        integer(),
        fun((gleam@http@request:request(AAOX)) -> gleam@http@response:response(AAOY)),
...
-type websocket_message(AAOZ) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, AAOZ}.
...
-type chunk_next(AAPA) :: {chunk_continue, AAPA} |
    chunk_stop |
    {chunk_abort, binary()}.

-file("src/mist.gleam", 53).
-spec continue(AAPB) -> next(AAPB, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 57).
-spec with_selector(next(AAPF, AAPG), gleam@erlang@process:selector(AAPG)) -> next(AAPF, AAPG).
...
-spec convert_next(next(AAPU, AAPV)) -> mist@internal@next:next(AAPU, AAPV).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(AAQW)) -> gleam@http@response:response(AAQY))
) -> builder(AAQW, AAQY).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(AARC, AARD), integer()) -> builder(AARC, AARD).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), AARI),
    integer(),
    gleam@http@response:response(AARI)
) -> builder(mist@internal@http:connection(), AARI).
...
-spec bind(builder(AARU, AARV), binary()) -> builder(AARU, AARV).
...
-spec with_ipv6(builder(AASA, AASB)) -> builder(AASA, AASB).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(AASG, AASH), binary(), binary()) -> builder(AASG, AASH).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(AASW)
) -> {ok, websocket_message(AASW)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((AATC, websocket_message(AATD), mist@internal@websocket:websocket_connection()) -> next(AATC, AATD)),
    fun((mist@internal@websocket:websocket_connection()) -> {AATC,
        gleam@option:option(gleam@erlang@process:selector(AATD))}),
    fun((AATC) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(AATR)) -> AATT),
    fun((AATT, AATR, s_s_e_connection()) -> gleam@otp@actor:next(AATT, AATR))
) -> gleam@http@response:response(response_data()).
...
-spec chunk_continue(AAUJ) -> chunk_next(AAUJ).
...
-spec chunked(
    gleam@http@request:request(mist@internal@http:connection()),
    gleam@http@response:response(any()),
    fun((gleam@erlang@process:subject(AAUC)) -> AAUE),
    fun((AAUE, AAUC, mist@internal@http:connection()) -> chunk_next(AAUE))
) -> gleam@http@response:response(response_data()).
