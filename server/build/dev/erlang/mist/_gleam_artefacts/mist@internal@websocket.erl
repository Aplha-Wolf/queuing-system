...
-type valid_message(AAAS) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, AAAS}.

-type websocket_message(AAAT) :: {valid, valid_message(AAAT)} | invalid.
...
-type handler_message(AAAU) :: {internal, gramps@websocket:frame()} |
    {user, AAAU}.

-type websocket_state(AAAV) :: {websocket_state,
        bitstring(),
        AAAV,
...
-spec map_user_selector(
    gleam@option:option(gleam@erlang@process:selector(AABZ))
) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(AABZ))).
...
-spec apply_frames(
    ...
    fun((AABP, handler_message(AABQ), websocket_connection()) -> mist@internal@next:next(AABP, AABQ)),
    websocket_connection(),
    mist@internal@next:next(AABP, websocket_message(AABQ)),
    fun((AABP) -> nil)
) -> mist@internal@next:next(AABP, websocket_message(AABQ)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {AABE,
        gleam@option:option(gleam@erlang@process:selector(AABF))}),
    fun((AABE) -> nil),
    fun((AABE, handler_message(AABF), websocket_connection()) -> mist@internal@next:next(AABE, AABF)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
