...
-spec method_override(gleam@http@request:request(ALHF)) -> gleam@http@request:request(ALHF).
...
-spec sort_keys(list({binary(), ALIW})) -> list({binary(), ALIW}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_500({ok, ALHQ} | {error, any()}) -> {ok, ALHQ} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
...
-spec fn_with_bad_request_error(
    fun((ALIE) -> {ok, ALIF} | {error, any()}),
    binary()
) -> fun((ALIE) -> {ok, ALIF} | {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((ALHY, bitstring()) -> {ok, ALHY} |
        {error, gleam@http@response:response(body())}),
    ALHY
) -> {ok, {gleam@option:option(buffered_reader()), integer(), ALHY}} |
    {error, gleam@http@response:response(body())}.
