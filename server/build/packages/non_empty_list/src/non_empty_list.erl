...
-type non_empty_list(DKT) :: {non_empty_list, DKT, list(DKT)}.
...
-spec any(non_empty_list(DLR), fun((DLR) -> boolean())) -> boolean().
any(List, Predicate) ->
...
-spec first(non_empty_list(DLW)) -> DLW.
first(List) ->
...
-spec find(non_empty_list(DMD), fun((DMD) -> boolean())) -> {ok, DMD} |
    {error, nil}.
...
-spec fold(non_empty_list(DMU), DMW, fun((DMW, DMU) -> DMW)) -> DMW.
...
-spec last(non_empty_list(DNU)) -> DNU.
last(List) ->
...
-spec new(DOU, list(DOU)) -> non_empty_list(DOU).
new(First, Rest) ->
...
-spec append_list(non_empty_list(DLN), list(DLN)) -> non_empty_list(DLN).
...
-spec from_list(list(DMX)) -> {ok, non_empty_list(DMX)} | {error, nil}.
from_list(List) ->
    case List of
        [] ->
            {error, nil};
...
-spec index_map(non_empty_list(DNI), fun((integer(), DNI) -> DNK)) -> non_empty_list(DNK).
index_map(List, Fun) ->
    new(
...
-spec intersperse(non_empty_list(DNR), DNR) -> non_empty_list(DNR).
...
-spec map(non_empty_list(DNY), fun((DNY) -> DOA)) -> non_empty_list(DOA).
map(List, Fun) ->
    new(
...
-spec prepend(non_empty_list(DOX), DOX) -> non_empty_list(DOX).
prepend(List, Item) ->
...
-spec reduce(non_empty_list(DPA), fun((DPA, DPA) -> DPA)) -> DPA.
reduce(List, Fun) ->
...
-spec rest(non_empty_list(DPC)) -> list(DPC).
rest(List) ->
    erlang:element(3, List).
...
-spec single(DPP) -> non_empty_list(DPP).
single(First) ->
    new(First, []).
...
-spec to_list(non_empty_list(DQE)) -> list(DQE).
to_list(Non_empty) ->
...
-spec append(non_empty_list(DLJ), non_empty_list(DLJ)) -> non_empty_list(DLJ).
append(First, Second) ->
    new(
        erlang:element(2, First),
...
-spec drop(non_empty_list(DLT), integer()) -> list(DLT).
drop(List, N) ->
    _pipe = List,
...
-spec reverse_and_prepend(non_empty_list(DMQ), non_empty_list(DMQ)) -> non_empty_list(DMQ).
...
-spec group(non_empty_list(DNC), fun((DNC) -> DNE)) -> gleam@dict:dict(DNE, non_empty_list(DNC)).
group(List, Key) ->
    _pipe = List,
    _pipe@1 = to_list(_pipe),
...
-spec reverse(non_empty_list(DPF)) -> non_empty_list(DPF).
reverse(List) ->
...
-spec all_loop(list({ok, DLC} | {error, DLD}), DLC, list(DLC)) -> {ok,
        non_empty_list(DLC)} |
    {error, DLD}.
...
-spec all(non_empty_list({ok, DKU} | {error, DKV})) -> {ok, non_empty_list(DKU)} |
    {error, DKV}.
all(Results) ->
...
-spec do_flatten(list(non_empty_list(DML)), non_empty_list(DML)) -> non_empty_list(DML).
...
-spec flatten(non_empty_list(non_empty_list(DMH))) -> non_empty_list(DMH).
flatten(Lists) ->
...
-spec flat_map(non_empty_list(DLY), fun((DLY) -> non_empty_list(DMA))) -> non_empty_list(DMA).
flat_map(List, Fun) ->
    _pipe = List,
...
-spec do_map2(non_empty_list(DOI), list(DOK), list(DOM), fun((DOK, DOM) -> DOI)) -> non_empty_list(DOI).
...
-spec map2(non_empty_list(DOC), non_empty_list(DOE), fun((DOC, DOE) -> DOG)) -> non_empty_list(DOG).
...
-spec map_fold(non_empty_list(DOP), DOR, fun((DOR, DOP) -> {DOR, DOS})) -> {DOR,
    non_empty_list(DOS)}.
...
-spec scan(non_empty_list(DPI), DPK, fun((DPK, DPI) -> DPK)) -> non_empty_list(DPK).
...
-spec shuffle(non_empty_list(DPM)) -> non_empty_list(DPM).
shuffle(List) ->
...
-spec sort(non_empty_list(DPR), fun((DPR, DPR) -> gleam@order:order())) -> non_empty_list(DPR).
sort(List, Compare) ->
    Sorted@1 = case begin
        _pipe = List,
        _pipe@1 = to_list(_pipe),
...
-spec take(non_empty_list(DQB), integer()) -> list(DQB).
take(List, N) ->
    _pipe = List,
    _pipe@1 = to_list(_pipe),
...
-spec unique(non_empty_list(DQH)) -> non_empty_list(DQH).
unique(List) ->
    Unique@1 = case begin
        _pipe = List,
        _pipe@1 = to_list(_pipe),
...
-spec unzip(non_empty_list({DQK, DQL})) -> {non_empty_list(DQK),
    non_empty_list(DQL)}.
unzip(List) ->
...
-spec zip(non_empty_list(DQP), non_empty_list(DQR)) -> non_empty_list({DQP, DQR}).
zip(List, Other) ->
    new(
...
-spec strict_zip(non_empty_list(DPU), non_empty_list(DPW)) -> {ok,
        non_empty_list({DPU, DPW})} |
    {error, nil}.
