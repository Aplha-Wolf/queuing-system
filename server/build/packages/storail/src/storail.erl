...
-type collection(ELY) :: {collection,
        binary(),
        fun((ELY) -> gleam@json:json()),
        gleam@dynamic@decode:decoder(ELY),
        config()}.

-type key(ELZ) :: {key, collection(ELZ), list(binary()), binary()}.
...
-spec key(collection(EMA), binary()) -> key(EMA).
key(Collection, Id) ->
    {key, Collection, [], Id}.

-file("src/storail.gleam", 86).
-spec namespaced_key(collection(EMD), list(binary()), binary()) -> key(EMD).
...
-spec parse_json(bitstring(), binary(), gleam@dynamic@decode:decoder(ENC)) -> {ok,
        ENC} |
    {error, storail_error()}.
...
-spec write(key(EMQ), EMQ) -> {ok, nil} | {error, storail_error()}.
write(Key, Data) ->
...
-spec move(key(EMU), key(EMU)) -> {ok, nil} | {error, storail_error()}.
...
-spec read(key(ENG)) -> {ok, ENG} | {error, storail_error()}.
read(Key) ->
...
-spec optional_read(key(ENK)) -> {ok, gleam@option:option(ENK)} |
    {error, storail_error()}.
optional_read(Key) ->
    Path = object_data_path(Key),
...
-spec read_namespace(collection(ENT), list(binary())) -> {ok,
        gleam@dict:dict(binary(), ENT)} |
    {error, storail_error()}.
